...
# ADDED: cmake is required to build lightgbm with GPU support.
sudo apt-get install -y libgomp1 nvidia-driver-535 cmake
...
# MODIFIED: Install a GPU-enabled build of lightgbm.
pip install pandas pyarrow gcsfs google-cloud-storage scikit-learn "lightgbm[gpu]" fuzzywuzzy optuna matplotlib seaborn python-Levenshtein tqdm shap tensorflow
...
def train_final_model(df_train_raw, df_tune_raw, feature_sets, specialist_config, universal_cols_present):
    ...
    def objective_single(trial):
        params = {
            ...
            'random_state': 42, 'n_jobs': -1, 'device': 'gpu'
        }
    ...
    for fold, (train_idx, val_idx) in enumerate(kf_final.split(X_fusion_main_aug)):
        ...
        model = lgb.LGBMRegressor(n_estimators=2500, **best_fusion_params, device='gpu', random_state=42, n_jobs=-1)
    ...
    final_fusion_model = lgb.LGBMRegressor(n_estimators=2500, **best_fusion_params, device='gpu', random_state=42, n_jobs=-1)
...
def generate_shap_reports_for_holdout(df_main_raw, df_holdout_raw, trained_models, feature_sets, universal_cols_present, holdout_results_df, output_dir):
    ...
    def get_augmented_features(df):
        ...
        # All columns are now numeric; we just need to exclude the explicit identifiers and target.
...
def predict_on_holdout(df_holdout_raw, trained_models, feature_sets, universal_cols_present):
    ...
    print("  - Generating final predictions from fusion model...")
    ...
    #   Augment with expert interaction features
...
def main():
    ...
    SPECIALIST_LGBM_CONFIG = {
        'head_A_dna_raw': {'n_estimators': 1000, 'num_leaves': 31, 'learning_rate': 0.05, 'random_state': 42, 'n_jobs': -1, 'colsample_bytree': 0.8, 'device': 'gpu'},
        'head_C_census': {'n_estimators': 1000, 'num_leaves': 31, 'learning_rate': 0.05, 'random_state': 42, 'n_jobs': -1, 'colsample_bytree': 0.8, 'device': 'gpu'},
        'head_compass_raw': {'n_estimators': 1500, 'num_leaves': 61, 'learning_rate': 0.03, 'random_state': 42, 'n_jobs': -1, 'colsample_bytree': 0.7, 'device': 'gpu'},
        'head_B_aesthetic': {'n_estimators': 1000, 'num_leaves': 31, 'learning_rate': 0.05, 'random_state': 42, 'n_jobs': -1, 'colsample_bytree': 0.8, 'device': 'gpu'},
        'head_A_dna_engineered': {'n_estimators': 1200, 'num_leaves': 41, 'learning_rate': 0.04, 'random_state': 42, 'n_jobs': -1, 'colsample_bytree': 0.7, 'device': 'gpu'},
        'head_G_gemini_quantitative': {'n_estimators': 1200, 'num_leaves': 41, 'learning_rate': 0.04, 'random_state': 42, 'n_jobs': -1, 'colsample_bytree': 0.7, 'device': 'gpu'},
    }
