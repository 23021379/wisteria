...
def main():
    ...
    # --- [A.D-V71.0] STAGE 7: THE V71 CHIMERA ARCHITECTURE LOCK-IN ---
    v71_model_package = train_v71_architecture(
        df_main_raw, feature_sets, universal_cols, 
        full_feature_sets_for_diagnostics, CANONICAL_METADATA_COLS, OUTPUT_DIR
    )
    joblib.dump(v71_model_package, os.path.join(OUTPUT_DIR, "v71_model_package.joblib"))

    print("\n--- STAGE 8: Final Validation & Reporting (V71) ---")
    if not df_holdout_raw.empty:
        holdout_results = predict_on_holdout_v71(df_holdout_raw, v71_model_package)
        ...
        print(f"--- HOLDOUT SET FINAL RESULTS (V71.0): MAE £{final_mae_holdout:,.2f} ---")
        holdout_results.to_csv(os.path.join(OUTPUT_DIR, "v71_holdout_results.csv"), index=False)
        
        print("\n--- STAGE 9: Generating V71 Diagnostic Omnibus Report ---")
        # generate_v71_diagnostic_omnibus_report(holdout_results, df_holdout_raw, v71_model_package, OUTPUT_DIR)
...
def train_v71_architecture(df_main_raw, feature_sets, universal_cols, full_feature_sets_for_diagnostics, non_feature_cols, output_dir):
    """
    [V71.0] Orchestrates the Chimera protocol, including the Conceptual Embedding Engine
    and the final augmented predictor model.
    """
    print("\n--- [V71.0] Engaging Chimera Protocol Training Orchestrator ---")

    # --- 1. & 2. Train Generalist, Sentinel, and partition data (same as V69) ---
    ...
    if len(df_generalist_candidates) > 100:
        generalist_model, _ = train_v18_residual_ensemble(
            df_generalist_candidates, feature_sets, universal_cols, output_dir, is_sub_model=True, domain_name="v71_generalist"
        )
    ...
    # --- 3. Train Anomaly Corrector (same as V69) ---
    ...
    # --- 4. Train Chimera Components ---
    embedding_package, chimera_predictor_package = None, None
    if not df_generalist_train.empty and not df_impoverished_train.empty:
        embedding_package = train_conceptual_embedding_model(df_generalist_train, universal_cols)
        
        all_feature_cols = [c for c in df_generalist_train.columns if c not in non_feature_cols]
        df_impoverished_augmented = augment_with_conceptual_twin_features(
            df_impoverished_train, df_generalist_train, embedding_package, all_feature_cols
        )
        chimera_predictor_package = train_chimera_predictor(df_impoverished_augmented, non_feature_cols)

    # --- 5. Package the Final Artifact (V71 Contract) ---
    v71_model_package = {
        ...
        'df_generalist_train_library': df_generalist_train, # CRITICAL: The reference library
        'contracts': {'universal_cols': universal_cols, 'all_feature_cols': all_feature_cols},
        'architecture_version': 'V71.0_Chimera'
    }
    return v71_model_package

def predict_on_holdout_v71(df_holdout_raw, v71_model_package):
    """[V71.0] Implements the Chimera three-path inference protocol."""
    print("\n--- [V71.0] Engaging Chimera Three-Path Inference Router ---")
    if df_holdout_raw.empty: return pd.DataFrame()

    generalist = v71_model_package['generalist_model']
    sentinel = v71_model_package['sentinel_anomaly_detector']
    corrector = v71_model_package['anomaly_corrector_model']
    chimera_predictor = v71_model_package['chimera_predictor_model']
    embedding_package = v71_model_package['embedding_package']
    df_library = v71_model_package['df_generalist_train_library']
    contracts = v71_model_package['contracts']
    ...
    # --- PATH 1: CHIMERA PREDICTOR (IMPOVERISHED) ---
    if impoverished_mask.any() and chimera_predictor and embedding_package and not df_library.empty:
        print(f"  - Routing {impoverished_mask.sum()} properties to Chimera Predictor...")
        df_impoverished_holdout = df_holdout_raw[impoverished_mask]
        df_augmented_holdout = augment_with_conceptual_twin_features(
            df_impoverished_holdout, df_library, embedding_package, contracts['all_feature_cols']
        )
        X_chimera = df_augmented_holdout[chimera_predictor['feature_contract']].fillna(0)
    ...
    if unpredicted_mask.any() and generalist:
        ...
        # Ensure the routing decision reflects the fallback action
...
def augment_with_conceptual_twin_features(df_query, df_reference, embedding_package, all_feature_cols):
    """[V71.0] Augments a query dataframe with the full feature vector of its conceptual twin."""
    print(f"\n--- [V71.0] Augmenting {len(df_query)} properties with conceptual twin data ---")
    
    encoder = embedding_package['encoder']
    scaler = embedding_package['scaler']
    feature_subset = embedding_package['feature_subset']
    
    X_ref_scaled = scaler.transform(df_reference[feature_subset].fillna(0))
    with torch.no_grad():
        reference_embeddings = encoder(torch.FloatTensor(X_ref_scaled).to(DEVICE)).cpu().numpy()

    X_query_scaled = scaler.transform(df_query[feature_subset].fillna(0))
    with torch.no_grad():
        query_embeddings = encoder(torch.FloatTensor(X_query_scaled).to(DEVICE)).cpu().numpy()

    nn_model = NearestNeighbors(n_neighbors=1, algorithm='auto').fit(reference_embeddings)
    distances, twin_indices = nn_model.kneighbors(query_embeddings)
    twin_indices = twin_indices.flatten()
    
    df_twins = df_reference[all_feature_cols].iloc[twin_indices].copy()
    df_twins.columns = ['twin_' + str(col) for col in df_twins.columns]
    
    df_query_augmented = pd.concat([df_query.reset_index(drop=True), df_twins.reset_index(drop=True)], axis=1)
    
    print(f"  - Augmentation complete. New feature count: {len(df_query_augmented.columns)}")
    return df_query_augmented

# === END ARCHITECTURAL DIRECTIVE A.D-V71.0 IMPLEMENTATION ===

def train_chimera_predictor(df_augmented_train, non_feature_cols):
    """
    [V71.0] Trains the final predictor on the augmented data from the Chimera engine.
    """
    print("\n--- [V71.0] Chimera Head III: Training the Chimera Predictor ---")
    if df_augmented_train.empty:
        print("  - No augmented data. Chimera Predictor will be disabled.")
        return None

    y_chimera = np.log1p(df_augmented_train['most_recent_sale_price'])
    
    # The feature contract is now all columns except the non-feature ones.
    feature_contract = [c for c in df_augmented_train.columns if c not in non_feature_cols]
    X_chimera = df_augmented_train[feature_contract].fillna(0)

    predictor_model = lgb.LGBMRegressor(
        n_estimators=1000, learning_rate=0.015, num_leaves=21,
        random_state=42, n_jobs=-1
    )
    predictor_model.fit(X_chimera, y_chimera)
    print("  - Chimera Predictor training complete.")
    
    return {'model': predictor_model, 'feature_contract': feature_contract}
