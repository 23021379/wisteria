...
# IMPORTANT: This string contains the full, structured persona evaluation framework as per Directive A-07.16.
persona_details_string = """
*   **Persona 1: FTB (Solo).**
    *   **Core Need:** Affordable independence.
    *   **Visual Focus:** Manageable size (1-2 beds), functional kitchen/bathroom, cosmetic updates are acceptable. Avoid major structural work.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Functional kitchen/bathroom (visuals), property `type` is 'Flat' or 'House' with 1-2 `bedrooms`.
    *   **Highly Desirable Factors (Significant Positives):** Easy commute (`rail_station_dist_km` < 2), `tenure` is 'Freehold' (if house).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Cosmetic updates needed (visuals), older `construction_year`.
    *   **Absolute Avoids (Major Negatives):** Major structural issues (visuals), poor energy efficiency (`epc_rating_current` < 60).

*   **Persona 2: Young Family (infants).**
    *   **Core Need:** Safe, supportive environment for young children.
    *   **Visual Focus:** Secure, enclosed garden (critical), family-friendly layout, adequate storage.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** `primary_school_ofsted` is 'Outstanding' or 'Good'; `deprivation_crime` decile > 5 (i.e. low crime).
    *   **Highly Desirable Factors (Significant Positives):** High `safety_index_100` (>75), high `family_index_100` (>75), 3-4 `bedrooms`.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Minor cosmetic updates (visuals).
    *   **Absolute Avoids (Major Negatives):** Poor school ratings, high crime areas (`deprivation_crime` < 4), no safe garden.

*   **Persona 3: Downsizers (Retired).**
    *   **Core Need:** Low maintenance, convenience, and healthcare access.
    *   **Visual Focus:** Bungalow or modern, ground-floor flat. Walk-in showers and minimal garden upkeep.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** No stairs (visuals, `property_sub_type` 'Bungalow' or `type` 'Flat'), `gp_surgery_dist_km` < 1.
    *   **Highly Desirable Factors (Significant Positives):** `tenure` is 'Freehold', high `safety_index_100`, high proportion of `employment_pct_retired` in area, low `deprivation_health` decile.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Smaller `floor_area_sqm` if well-designed.
    *   **Absolute Avoids (Major Negatives):** High upkeep (visuals, old `construction_year`), poor access to amenities (`bus_stop_dist_km` > 1).

*   **Persona 4: Remote Worker (Single).**
    *   **Core Need:** A dedicated, productive work environment with a good quality of life.
    *   **Visual Focus:** A distinct room for a quiet office with good natural light.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Dedicated office space (visuals).
    *   **Highly Desirable Factors (Significant Positives):** High `life_quality_index_100` (>80), low `deprivation_environment` decile (>6), character (visuals).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Further from city centers (`rail_station_dist_km` > 3).
    *   **Absolute Avoids (Major Negatives):** No viable dedicated workspace (visuals), poor living environment (`deprivation_environment` < 4).

*   **Persona 5: FTB Couple.**
    *   **Core Need:** Escape rent, future space for growth.
    *   **Visual Focus:** 2+ `bedrooms`, functional kitchen/bathroom, some outdoor space. Cosmetic updates are acceptable.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** At least 2 `bedrooms`, functional kitchen/bath (visuals).
    *   **Highly Desirable Factors (Significant Positives):** `tenure` is 'Freehold', scope for cosmetic updates (visuals), good overall condition (visuals).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Requires redecoration, leasehold flat (`tenure` is 'Leasehold/Other').
    *   **Absolute Avoids (Major Negatives):** Major immediate structural repairs (visuals), `bedrooms` < 2.

*   **Persona 6: Education Parents (School Age).**
    *   **Core Need:** The BEST possible secondary school catchment.
    *   **Visual Focus:** 3-4+ `bedrooms`, ample space for study, secure garden.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** `secondary_school_ofsted` is 'Outstanding' or 'Good', low `deprivation_crime` decile (>5).
    *   **Highly Desirable Factors (Significant Positives):** Low `deprivation_education` decile (>7), dedicated study space (visuals), high `family_index_100`.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Interior aesthetics can be updated later.
    *   **Absolute Avoids (Major Negatives):** Poor secondary school ratings, unsafe areas (`safety_index_100` < 60).

*   **Persona 7: Active Retiree (Widowed).**
    *   **Core Need:** Manageable living, social access, security.
    *   **Visual Focus:** 2 `bedrooms`, low maintenance property and garden.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** No stairs (visuals, `sub_type` is 'Bungalow'), manageable garden (visuals).
    *   **Highly Desirable Factors (Significant Positives):** High `safety_index_100`, low `gp_surgery_dist_km`, established community (`average_resident_age` > 40).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** `tenure` is 'Leasehold/Other' if in a well-managed block.
    *   **Absolute Avoids (Major Negatives):** Isolation (`bus_stop_dist_km` > 1.5), high-crime area (`deprivation_crime` < 5).

*   **Persona 8: Eco-Conscious (Single).**
    *   **Core Need:** Sustainable and low-impact living.
    *   **Visual Focus:** Garden space for growing, modern windows/glazing, south-facing roof for solar potential.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** `epc_rating_current` is 75+ (High C or above).
    *   **Highly Desirable Factors (Significant Positives):** High `epc_rating_potential` (>92 for A rating), near public transport (`bus_stop_dist_km` < 0.5), newer `construction_year` (>2000).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Cosmetic updates needed.
    *   **Absolute Avoids (Major Negatives):** Low `epc_rating_current` (< 60), poor public transport access.

*   **Persona 9: Fixer-Upper Fans (Couple, skilled).**
    *   **Core Need:** Renovation potential to build equity.
    *   **Visual Focus:** Structurally sound but dated interiors. Avoid signs of major structural flaws.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Appears structurally sound (visuals), clear renovation potential (visuals).
    *   **Highly Desirable Factors (Significant Positives):** Good location (`life_quality_index_100` > 70), older `construction_year` (<1980), poor `epc_rating_current` (indicates upgrade potential).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Very dated interiors.
    *   **Absolute Avoids (Major Negatives):** Evidence of major structural flaws (visuals), bad location (`deprivation_environment` < 4).

*   **Persona 10: Established Family (Teens).**
    *   **Core Need:** Space and dedicated zones for teens, good schooling.
    *   **Visual Focus:** 4-5 `bedrooms`, separate teen area (social/study), good kitchen/diner.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** At least 4 `bedrooms`, `secondary_school_ofsted` is 'Outstanding' or 'Good'.
    *   **Highly Desirable Factors (Significant Positives):** High `floor_area_sqm` (>120), separate teen space (visuals), low `deprivation_education` decile (>7).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Garden size if interior space is generous.
    *   **Absolute Avoids (Major Negatives):** Cramped layout (`floor_area_sqm` < 100 with 4 beds), unsafe area (`deprivation_crime` < 5).

*   **Persona 11: Upsizers (Young Child + Expecting).**
    *   **Core Need:** More space for growing family, dedicated WFH.
    *   **Visual Focus:** 4 `bedrooms`, good-sized garden, dedicated home office, modern kitchen/bathroom.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** At least 4 `bedrooms`, dedicated home office (visuals).
    *   **Highly Desirable Factors (Significant Positives):** High `family_index_100`, good `primary_school_ofsted`, large garden (visuals).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Minor cosmetic updates.
    *   **Absolute Avoids (Major Negatives):** No viable WFH space, small `floor_area_sqm`, poor school district.

*   **Persona 12: Urban Pro Couple (DINK).**
    *   **Core Need:** Central, high-spec, low maintenance living.
    *   **Visual Focus:** High-spec modern finishes, good entertaining layout, apartment/townhouse style.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Modern finishes (visuals), low maintenance (`type` 'Flat' or newer `construction_year`).
    *   **Highly Desirable Factors (Significant Positives):** Prime location (`rail_station_dist_km` < 1), high `life_quality_index_100`, youthful area (`average_resident_age` < 40).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Smaller outdoor space if a balcony is present.
    *   **Absolute Avoids (Major Negatives):** Long commutes (`rail_station_dist_km` > 2), dated finishes (visuals).

*   **Persona 13: Luxury Seeker (HNWI).**
    *   **Core Need:** Statement property, status, and privacy.
    *   **Visual Focus:** Unique/bespoke design, high-end finishes, premium amenities (views, pool).
    *   **Critical Must-Haves (Absolute Deal-Breakers):** High-end finishes (visuals), premium amenities (visuals).
    *   **Highly Desirable Factors (Significant Positives):** Unique design (visuals), excellent privacy (visuals), affluent area (top `deprivation_environment` decile).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** None; requires excellence across the board.
    *   **Absolute Avoids (Major Negatives):** Generic finishes (visuals), compromised privacy.

*   **Persona 14: Investor (BTL).**
    *   **Core Need:** Strong tenant demand, minimal maintenance.
    *   **Visual Focus:** Durable, easy-to-clean surfaces, simple modern kitchen/bathroom, good general condition.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Good condition (visuals), `bedrooms` >= 2.
    *   **Highly Desirable Factors (Significant Positives):** Strong rental market (high `employment_pct_full_time` or `employment_pct_student`), close to transport (`rail_station_dist_km`), newer `construction_year`.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Basic decor, `tenure` is 'Leasehold/Other'.
    *   **Absolute Avoids (Major Negatives):** Poor condition requiring investment, niche appeal, poor transport links.

*   **Persona 15: Multi-Generational Household.**
    *   **Core Need:** Shared living with privacy for all.
    *   **Visual Focus:** 5+ `bedrooms`, multiple living areas, potential for ground floor suite/annex.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** At least 5 `bedrooms`, multiple distinct living areas (visuals).
    *   **Highly Desirable Factors (Significant Positives):** High `floor_area_sqm` (>150), potential for ground floor suite (visuals), low `deprivation_health` decile.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Cosmetic updates needed.
    *   **Absolute Avoids (Major Negatives):** Lack of privacy/separation, poor layout for mixed ages.

*   **Persona 16: Pet Owner (Single + Active Dog).**
    *   **Core Need:** Suitable home for an active dog.
    *   **Visual Focus:** Secure enclosed garden/patio, durable/easy-clean floors, easy garden access.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Secure enclosed garden/patio (visuals), durable flooring (visuals).
    *   **Highly Desirable Factors (Significant Positives):** Near parks/walks (high `deprivation_environment` decile), `hospital_dist_km` is low (proxy for vet access).
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Smaller living space if outdoor space is good.
    *   **Absolute Avoids (Major Negatives):** No private outdoor space, `type` is 'Flat' with no direct garden access.

*   **Persona 17: Social Entertainers (Couple).**
    *   **Core Need:** Ample space for frequent, large-scale hosting.
    *   **Visual Focus:** Large open-plan kitchen/living/dining, high-spec kitchen, good indoor/outdoor flow.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Large open-plan entertaining space (visuals), functional outdoor entertaining space (visuals).
    *   **Highly Desirable Factors (Significant Positives):** High-spec kitchen (visuals), high `floor_area_sqm` (>130), good `life_quality_index_100`.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Smaller individual rooms if main spaces are ample.
    *   **Absolute Avoids (Major Negatives):** Segregated kitchen/dining, poor guest flow.

*   **Persona 18: Minimalist (Single).**
    *   **Core Need:** Calm, uncluttered, highly functional space.
    *   **Visual Focus:** Clean lines, excellent built-in storage, good natural light, simple high-quality finishes.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Excellent built-in storage (visuals), simple high-quality finishes (visuals).
    *   **Highly Desirable Factors (Significant Positives):** Good natural light (visuals), newer `construction_year`, high `life_quality_index_100`.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Smaller `floor_area_sqm`.
    *   **Absolute Avoids (Major Negatives):** Lack of built-in storage, ornate/dated details.

*   **Persona 19: Creative/Artist (Single Parent).**
    *   **Core Need:** Home with dedicated studio space.
    *   **Visual Focus:** Large room usable as a studio with good natural light, separate living area, storage.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** Viable studio room with good light (visuals), separate main living area.
    *   **Highly Desirable Factors (Significant Positives):** Character/inspiration (visuals), good `primary_school_ofsted`.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** Requires redecoration.
    *   **Absolute Avoids (Major Negatives):** No dedicated workspace, poor natural light.

*   **Persona 20: Relocators (Couple, New Area).**
    *   **Core Need:** Reliable, safe, convenient, and move-in ready home.
    *   **Visual Focus:** Excellent, move-in-ready condition. No visible signs of immediate repairs.
    *   **Critical Must-Haves (Absolute Deal-Breakers):** High `safety_index_100` (>75), low `deprivation_crime` decile (>5), move-in ready condition (visuals).
    *   **Highly Desirable Factors (Significant Positives):** High `life_quality_index_100`, good transport links (`rail_station_dist_km`), good school ratings.
    *   **Acceptable Compromises (Minor Negatives/Tolerances):** An older `construction_year` if well-maintained.
    *   **Absolute Avoids (Major Negatives):** Any signs of major repairs (visuals), areas with poor safety/crime metrics.
"""
persona_start_pattern = r'\*\s+Persona\s+\d+:' # Match this pattern, not persona_start_pattern_match
personas_raw = re.split(f'[REDACTED_BY_SCRIPT]', persona_details_string.strip()) # Use persona_details_string
...
# Prompt 5: Master Template for Micro-Batch Evaluation
prompt5_micro_batch_template = """
**Goal:** For the **{persona_range_identifier} ONLY**, provide suitability ratings and justifications for the overall property and each evaluated room. You must also provide a general, non-persona-specific attractiveness rating for each room.

**Inputs:**
1.  **Property & Room Data (JSON):** The following JSON objects contain all previously analyzed visual and contextual data for the property.
    *   Room Assignments: ```json\n{room_assignment_data_json}\n```
    *   Room Features: ```json\n{room_feature_data_json}\n```
    *   Room Evaluations (Flaws/SPs): ```json\n{room_evaluation_data_json}\n```2.  **Room Images:** Provided contextually (Image 1, Image 2...).
3.  **Persona Details (Subset - {persona_range_identifier}):** Provided below.

**Task Steps (Execute in Order):**
1.  **General Room Rating:** For each room `label` in the evaluation data, generate an **Overall Attractiveness Rating (1-10)** (10=Modern/Appealing, 1=Dated/Poor). This rating is general and NOT specific to any persona.
2.  **Individual Persona Evaluation (Loop through {persona_range_identifier} ONLY):** For **each** of the personas detailed below:
    *   Provide an overall **Property Suitability Rating (1-10)** and a brief **Justification**.
    *   For **each evaluated room**, provide a **Room Suitability Rating (1-10)** and a brief **Justification**.
    *   **Constraint:** All justifications MUST integrate reasoning from BOTH visual evidence and the provided `Property Context Block`. No furniture mentions. No monetary figures.

**Persona Details & Checklists ({persona_range_identifier}):**
{persona_details_subset}

**Output Format:**
Provide ONLY a JSON object (no introduction or backticks) with the exact structure below.
*   `evaluated_room_labels`: List of evaluated room labels.
*   `room_ratings`: List of lists. Each inner list corresponds to a room and MUST contain **6** elements: `[General Attractiveness Rating, P{start_num} Rating, ..., P{end_num} Rating]`.
*   `overall_property_ratings_by_persona`: An object mapping the 5 specific persona identifiers to their overall property rating and justification.
"""


# Split the full persona details string into four chunks of five personas each.
personas_p1_5 = "\n\n".join(personas[:5])
personas_p6_10 = "\n\n".join(personas[5:10])
personas_p11_15 = "\n\n".join(personas[10:15])
personas_p16_20 = "\n\n".join(personas[15:20])

# Instantiate the four new prompts from the master template
prompt5a_p1_5 = prompt5_micro_batch_template.format(
    persona_range_identifier="Personas 1-5",
    persona_details_subset="[REDACTED_BY_SCRIPT]",
    start_num=1, end_num=5,
    room_assignment_data_json="[REDACTED_BY_SCRIPT]",
    room_feature_data_json="[REDACTED_BY_SCRIPT]",
    room_evaluation_data_json="[REDACTED_BY_SCRIPT]"
)
prompt5b_p6_10 = prompt5_micro_batch_template.format(
    persona_range_identifier="Personas 6-10",
    persona_details_subset="[REDACTED_BY_SCRIPT]",
    start_num=6, end_num=10,
    room_assignment_data_json="[REDACTED_BY_SCRIPT]",
    room_feature_data_json="[REDACTED_BY_SCRIPT]",
    room_evaluation_data_json="[REDACTED_BY_SCRIPT]"
)
prompt5c_p11_15 = prompt5_micro_batch_template.format(
    persona_range_identifier="Personas 11-15",
    persona_details_subset="[REDACTED_BY_SCRIPT]",
    start_num=11, end_num=15,
    room_assignment_data_json="[REDACTED_BY_SCRIPT]",
    room_feature_data_json="[REDACTED_BY_SCRIPT]",
    room_evaluation_data_json="[REDACTED_BY_SCRIPT]"
)
prompt5d_p16_20 = prompt5_micro_batch_template.format(
    persona_range_identifier="Personas 16-20",
    persona_details_subset="[REDACTED_BY_SCRIPT]",
    start_num=16, end_num=20,
    room_assignment_data_json="[REDACTED_BY_SCRIPT]",
    room_feature_data_json="[REDACTED_BY_SCRIPT]",
    room_evaluation_data_json="[REDACTED_BY_SCRIPT]"
)
...
async def process_property_pipeline_for_worker(
    ...
    try: # Main pipeline steps
        ...
        # Step 5: Micro-Batch Persona Evaluation
        property_context_block_string = "{}"
        try:
            import gemini_property_feature_generator
            property_context_block_string = gemini_property_feature_generator.generate_property_context_block(property_address, latest_year)
            log_to_queue(current_process_log_q, logging.INFO, "[REDACTED_BY_SCRIPT]", property_id_with_year)
        except Exception as e_context:
            log_to_queue(current_process_log_q, logging.ERROR, "[REDACTED_BY_SCRIPT]", property_id_with_year, e_context, exc_info=True)
            property_context_block_string = json.dumps({"property_vitals": {}, "[REDACTED_BY_SCRIPT]": {}, "[REDACTED_BY_SCRIPT]": {}, "amenities_and_transport": {}}, indent=2)

        common_context_map = {
            "[REDACTED_BY_SCRIPT]": output2_text,
            "[REDACTED_BY_SCRIPT]": output3_text,
            "[REDACTED_BY_SCRIPT]": output4_text,
            "[REDACTED_BY_SCRIPT]": property_context_block_string
        }

        output5a_json, _ = await call_step_with_client_retry(
            prompt5a_p1_5, indexed_room_images, "[REDACTED_BY_SCRIPT]", model_25_pro, long_timeout,
            current_process_log_q, property_id_with_year,
            context_map={**common_context_map, "[REDACTED_BY_SCRIPT]": personas_p1_5}
        )
        if not output5a_json: raise StopIteration("[REDACTED_BY_SCRIPT]")

        output5b_json, _ = await call_step_with_client_retry(
            prompt5b_p6_10, indexed_room_images, "[REDACTED_BY_SCRIPT]", model_25_pro, long_timeout,
            current_process_log_q, property_id_with_year,
            context_map={**common_context_map, "[REDACTED_BY_SCRIPT]": personas_p6_10}
        )
        if not output5b_json: raise StopIteration("[REDACTED_BY_SCRIPT]")

        output5c_json, _ = await call_step_with_client_retry(
            prompt5c_p11_15, indexed_room_images, "[REDACTED_BY_SCRIPT]", model_25_pro, long_timeout,
            current_process_log_q, property_id_with_year,
            context_map={**common_context_map, "[REDACTED_BY_SCRIPT]": personas_p11_15}
        )
        if not output5c_json: raise StopIteration("[REDACTED_BY_SCRIPT]")

        output5d_json, _ = await call_step_with_client_retry(
            prompt5d_p16_20, indexed_room_images, "[REDACTED_BY_SCRIPT]", model_25_pro, long_timeout,
            current_process_log_q, property_id_with_year,
            context_map={**common_context_map, "[REDACTED_BY_SCRIPT]": personas_p16_20}
        )
        if not output5d_json: raise StopIteration("[REDACTED_BY_SCRIPT]")

        # New Merge Protocol for Micro-Batches
        try:
            sub_steps = [output5a_json, output5b_json, output5c_json, output5d_json]
            if not all(isinstance(s, dict) for s in sub_steps):
                raise ValueError("[REDACTED_BY_SCRIPT]")

            canonical_labels = sub_steps.get("[REDACTED_BY_SCRIPT]")
            if not isinstance(canonical_labels, list) or not canonical_labels:
                raise ValueError("[REDACTED_BY_SCRIPT]'evaluated_room_labels'.")

            for i, step_output in enumerate(sub_steps[1:], 1):
                if step_output.get("[REDACTED_BY_SCRIPT]") != canonical_labels:
                    raise ValueError(f"FATAL: Mismatch in 'evaluated_room_labels'[REDACTED_BY_SCRIPT]")

            merged_step5_output = {
                "[REDACTED_BY_SCRIPT]": canonical_labels,
                "[REDACTED_BY_SCRIPT]": {},
                "room_ratings_final": []
            }

            for step_output in sub_steps:
                overall_ratings = step_output.get("[REDACTED_BY_SCRIPT]", {})
                if isinstance(overall_ratings, dict):
                    merged_step5_output["[REDACTED_BY_SCRIPT]"].update(overall_ratings)

            num_rooms = len(canonical_labels)
            for room_idx in range(num_rooms):
                ratings_5a = sub_steps.get("room_ratings", [[]]*num_rooms)[room_idx]
                ratings_5b = sub_steps.get("room_ratings", [[]]*num_rooms)[room_idx][1:]
                ratings_5c = sub_steps.get("room_ratings", [[]]*num_rooms)[room_idx][1:]
                ratings_5d = sub_steps.get("room_ratings", [[]]*num_rooms)[room_idx][1:]

                if len(ratings_5a) != 6 or len(ratings_5b) != 5 or len(ratings_5c) != 5 or len(ratings_5d) != 5:
                    raise ValueError(f"[REDACTED_BY_SCRIPT]'{canonical_labels[room_idx]}'[REDACTED_BY_SCRIPT]")

                final_21_ratings = ratings_5a + ratings_5b + ratings_5c + ratings_5d
                persona_ratings_only = final_21_ratings[1:]
                ...
                merged_step5_output["room_ratings_final"].append(final_21_ratings + [average_rating])
        ...
        if pipeline_successful_for_csv_generation:
            ...
            if required_files_for_feat_gen_exist:
                try:
                    import gemini_property_feature_generator_2 # Ensure import
                    features_for_csv = gemini_property_feature_generator_2.process_property(
                ...
                except ImportError: log_to_queue(current_process_log_q, logging.CRITICAL, "[REDACTED_BY_SCRIPT]'gemini_property_feature_generator_2'.", property_id_with_year)
...
def main_multiprocessing_runner():
    ...
    try:
        import gemini_property_feature_generator_2 # Make sure this module is available
        csv_header_list = gemini_property_feature_generator_2.get_feature_header()
