...
#
#
## TRIAL 14 results: srgan-bucket-ace-botany-453819-t4/model_training/run_20250904-165407
#
#
#
#
...
pip install pandas pyarrow gcsfs google-cloud-storage scikit-learn lightgbm fuzzywuzzy optuna matplotlib seaborn python-Levenshtein tqdm shap xgboost
...
TRIAGE_THRESHOLD = float(os.environ.get("TRIAGE_THRESHOLD", 0.7))
...
#
# === BEGIN ARCHITECTURAL DIRECTIVE A.D-V23.0 IMPLEMENTATION ===
#

def _train_gatekeeper(df_main, universal_cols):
    """[V23.0] Trains the lightweight Gatekeeper model and identifies hard-to-predict samples."""
    print("\n--- V23.0 - Phase 1: Training Universalist Gatekeeper ---")
    X_gatekeeper = df_main[universal_cols].fillna(0)
    y_log = np.log1p(df_main['most_recent_sale_price'])
    
    oof_preds_log = np.zeros(len(df_main))
    kf = KFold(n_splits=NUM_FOLDS_FINAL, shuffle=True, random_state=42)
    
    params = {'n_estimators': 1500, 'learning_rate': 0.02, 'num_leaves': 31, 'reg_alpha': 0.1, 'reg_lambda': 0.1, 'colsample_bytree': 0.7, 'subsample': 0.7, 'random_state': 42, 'n_jobs': -1, 'device': 'cpu', 'verbosity': -1}

    for _, (train_idx, val_idx) in enumerate(tqdm(kf.split(X_gatekeeper), desc="Training Gatekeeper OOF")):
        model = lgb.LGBMRegressor(**params)
        model.fit(X_gatekeeper.iloc[train_idx], y_log.iloc[train_idx], eval_set=[(X_gatekeeper.iloc[val_idx], y_log.iloc[val_idx])], eval_metric='mae', callbacks=[lgb.early_stopping(50, verbose=False), lgb.log_evaluation(period=0)])
        oof_preds_log[val_idx] = model.predict(X_gatekeeper.iloc[val_idx])
        
    final_gatekeeper_model = lgb.LGBMRegressor(**params).fit(X_gatekeeper, y_log)
    
    difficulty_df = pd.DataFrame({
        'property_id': df_main['property_id'],
        'residual_difficulty_score': np.abs(y_log - oof_preds_log)
    })
    
    print("  - Gatekeeper training complete. Difficulty scores calculated from OOF predictions.")
    return final_gatekeeper_model, difficulty_df, oof_preds_log

def _train_specialist_council(df_hard_samples, y_hard_samples_residuals_log, feature_sets, output_dir):
    """[V23.0] Trains the L0 specialist models ONLY on the identified hard samples."""
    print("\n--- V23.0 - Phase 2: Training Specialist Council on High-Alert Samples ---")
    print(f"  - Council will train on {len(df_hard_samples)} most difficult samples.")
    
    dormant_heads_path = os.path.join(output_dir, "dormant_heads.json")
    dormant_heads = set()
    if os.path.exists(dormant_heads_path):
        with open(dormant_heads_path, 'r') as f:
            dormant_heads = set(json.load(f))
        print(f"  - Loaded dormant manifest. Deactivating {len(dormant_heads)} specialists.")

    trained_specialists = {}
    params = {'n_estimators': 1000, 'learning_rate': 0.03, 'num_leaves': 21, 'reg_alpha': 0.2, 'reg_lambda': 0.2, 'colsample_bytree': 0.7, 'subsample': 0.7, 'random_state': 42, 'n_jobs': -1, 'device': 'cpu', 'verbosity': -1}

    for head_name in tqdm(sorted(feature_sets.keys()), desc="Training Specialist Council"):
        if head_name in dormant_heads:
            continue
        cols = feature_sets[head_name]
        if not cols: continue
        
        X_specialist = df_hard_samples[cols].fillna(0)
        model = lgb.LGBMRegressor(**params).fit(X_specialist, y_hard_samples_residuals_log)
        trained_specialists[head_name] = model
        
    print(f"  - Specialist Council training complete. {len(trained_specialists)} active specialists trained.")
    return trained_specialists

def _create_arbiter_feature_matrix(df, gatekeeper_preds_log, specialist_council, feature_sets, universal_cols):
    """[V23.0] Constructs the full feature matrix for the TabNet Arbiter."""
    print("  - Constructing feature matrix for Arbiter...")
    
    # 1. Start with Gatekeeper's predictions
    arbiter_features = pd.DataFrame({'gatekeeper_pred_log': gatekeeper_preds_log}, index=df.index)
    
    # 2. Add predictions from all specialists
    for head_name, model in specialist_council.items():
        cols = feature_sets.get(head_name, [])
        if not cols: continue
        present_cols = [c for c in cols if c in df.columns]
        X_specialist = df[present_cols].fillna(0)
        missing_cols = set(cols) - set(present_cols)
        for col in missing_cols: X_specialist[col] = 0
        X_specialist = X_specialist[cols]
        arbiter_features[f'l0_resid_pred_{head_name}'] = model.predict(X_specialist)
        
    # 3. Add the raw universal features
    X_universal = df[universal_cols].fillna(0)
    
    final_matrix = pd.concat([arbiter_features, X_universal], axis=1)
    return final_matrix

def _train_xgboost_arbiter(df_main, gatekeeper_preds_log, specialist_council, feature_sets, universal_cols, integrity_cols):
    """[V24.0] Trains the robust XGBoost Arbiter on the full-spectrum data matrix."""
    print("\n--- V24.0 - Phase 3: Training Contextual XGBoost Arbiter ---")
    
    # THE CRITICAL CORRECTION: Generate predictions from specialists for the ENTIRE training set.
    print("  - Generating full-spectrum prediction matrix for Arbiter...")
    X_arbiter_full = _create_arbiter_feature_matrix(df_main, gatekeeper_preds_log, specialist_council, feature_sets, universal_cols)
    
    # Add the new integrity features
    X_arbiter_full = pd.concat([X_arbiter_full, df_main[integrity_cols]], axis=1)
    
    y_arbiter_full = np.log1p(df_main['most_recent_sale_price'])
    arbiter_feature_names = X_arbiter_full.columns.tolist()
    
    oof_preds_log = np.zeros(len(df_main))
    kf = KFold(n_splits=NUM_FOLDS_FINAL, shuffle=True, random_state=42)
    
    params = {'objective': 'reg:squarederror', 'n_estimators': 2000, 'learning_rate': 0.02, 'max_depth': 5, 'subsample': 0.7, 'colsample_bytree': 0.7, 'random_state': 42, 'n_jobs': -1, 'tree_method': 'hist', 'early_stopping_rounds': 50}

    for _, (train_idx, val_idx) in enumerate(tqdm(kf.split(X_arbiter_full), desc="Training XGBoost Arbiter OOF")):
        X_train, X_val = X_arbiter_full.iloc[train_idx], X_arbiter_full.iloc[val_idx]
        y_train, y_val = y_arbiter_full.iloc[train_idx], y_arbiter_full.iloc[val_idx]
        
        arbiter = xgb.XGBRegressor(**params)
        arbiter.fit(X_train, y_train, eval_set=[(X_val, y_val)], verbose=False)
        oof_preds_log[val_idx] = arbiter.predict(X_val)

    print("  - Training final Arbiter model on all data...")
    # Remove early stopping for the final model fit on all data
    final_params = params.copy()
    del final_params['early_stopping_rounds']
    final_arbiter_model = xgb.XGBRegressor(**final_params).fit(X_arbiter_full, y_arbiter_full)
    
    return final_arbiter_model, oof_preds_log, arbiter_feature_names

def run_contextual_arbiter_protocol(df_main_raw, feature_sets, universal_cols_present, output_dir):
    """
    [A.D-V24.0 ORCHESTRATOR]
    Implements the Contextual Arbiter Protocol, fixing the context-starvation issue of V23.
    """
    # PHASE 1: Train Gatekeeper and identify hard samples (Unchanged)
    gatekeeper_model, difficulty_df, gatekeeper_oof_preds_log = _train_gatekeeper(df_main_raw, universal_cols_present)
    
    # PHASE 2: Train Specialist Council on the hard samples (Unchanged)
    threshold_value = difficulty_df['residual_difficulty_score'].quantile(TRIAGE_THRESHOLD)
    hard_sample_ids = difficulty_df[difficulty_df['residual_difficulty_score'] >= threshold_value]['property_id']
    df_hard_samples = df_main_raw[df_main_raw['property_id'].isin(hard_sample_ids)].copy()
    
    y_log_main = np.log1p(df_main_raw['most_recent_sale_price'])
    gatekeeper_preds_on_main_log = gatekeeper_model.predict(df_main_raw[universal_cols_present].fillna(0))
    y_residuals_main_log = y_log_main - gatekeeper_preds_on_main_log
    
    hard_sample_indices = df_main_raw['property_id'].isin(hard_sample_ids)
    y_hard_samples_residuals_log = y_residuals_main_log[hard_sample_indices]

    specialist_council = _train_specialist_council(df_hard_samples, y_hard_samples_residuals_log, feature_sets, output_dir)
    
    # PHASE 2.5: Engineer Data Integrity Features for the full training set
    df_main_with_integrity, integrity_cols = engineer_data_integrity_features(df_main_raw, feature_sets, universal_cols_present)

    # PHASE 3: Train the XGBoost Arbiter on the FULL dataset with full context
    arbiter_model, arbiter_oof_preds_log, arbiter_feature_names = _train_xgboost_arbiter(
        df_main_with_integrity, gatekeeper_oof_preds_log, specialist_council, feature_sets, universal_cols_present, integrity_cols
    )

    # Assemble final model package
    trained_models = {
        'gatekeeper_model': gatekeeper_model,
        'specialist_council': specialist_council,
        'arbiter_model': arbiter_model,
        'universal_cols': universal_cols_present,
        'integrity_cols': integrity_cols,
        'feature_sets': feature_sets,
        'arbiter_feature_names': arbiter_feature_names,
        'architecture_version': 'V24.0_CONTEXTUAL_ARBITER'
    }
    
    eval_df = df_main_raw[['property_id', 'most_recent_sale_price']].copy()
    eval_df['final_predicted_price'] = np.expm1(arbiter_oof_preds_log)
    
    gatekeeper_mae = mean_absolute_error(np.expm1(y_log_main), np.expm1(gatekeeper_oof_preds_log))

    return trained_models, eval_df, None, None, gatekeeper_mae
...
def predict_on_holdout_v24(df_holdout_raw, trained_models):
    """[V24.0] Generates predictions on the holdout set using the Contextual Arbiter Protocol."""
    print("\n--- V24.0: Final Evaluation on True Holdout Set ---")
    if df_holdout_raw.empty: return pd.DataFrame()

    gatekeeper_model = trained_models['gatekeeper_model']
    specialist_council = trained_models['specialist_council']
    arbiter_model = trained_models['arbiter_model']
    universal_cols = trained_models['universal_cols']
    integrity_cols = trained_models['integrity_cols']
    feature_sets = trained_models['feature_sets']

    # Phase 1: Predict with Gatekeeper
    print("  - Phase 1: Predicting with Gatekeeper...")
    X_gatekeeper_holdout = df_holdout_raw[universal_cols].fillna(0)
    gatekeeper_preds_log = gatekeeper_model.predict(X_gatekeeper_holdout)

    # Phase 2.5: Engineer integrity features for the holdout set
    df_holdout_with_integrity, _ = engineer_data_integrity_features(df_holdout_raw, feature_sets, universal_cols)

    # Phase 3: Construct Arbiter features and predict
    print("  - Phase 3: Assembling features and predicting with XGBoost Arbiter...")
    X_arbiter_holdout = _create_arbiter_feature_matrix(
        df_holdout_with_integrity, gatekeeper_preds_log, specialist_council, feature_sets, universal_cols
    )
    X_arbiter_holdout = pd.concat([X_arbiter_holdout, df_holdout_with_integrity[integrity_cols]], axis=1)
    
    expected_arbiter_features = trained_models['arbiter_feature_names']
    X_arbiter_holdout = X_arbiter_holdout.reindex(columns=expected_arbiter_features, fill_value=0)

    final_preds_log = arbiter_model.predict(X_arbiter_holdout)
...
def main():
    ...
    df_main_raw, df_holdout_raw = train_test_split(df_final, test_size=0.05, random_state=42)
    ...
    print("\n--- STAGE 7: A.D-V24.0 Contextual Arbiter Protocol Training ---")
    trained_models, eval_df, specialist_perf_df, assembler_shap_artifacts, baseline_mae = run_contextual_arbiter_protocol(df_main_raw, feature_sets, universal_cols_present, OUTPUT_DIR)
    joblib.dump(trained_models, os.path.join(OUTPUT_DIR, "v24_contextual_arbiter_models.joblib"))
    ...
    print(f"--- OOF PERFORMANCE (V24.0 ASSEMBLY): MAE £{final_mae_oof:,.2f} ---")
    eval_df.to_csv(os.path.join(OUTPUT_DIR, "v24_oof_predictions.csv"), index=False)
    
    if not df_holdout_raw.empty:
        holdout_results = predict_on_holdout_v24(df_holdout_raw, trained_models)
        ...
        print(f"--- HOLDOUT SET FINAL RESULTS (V24.0): MAE £{final_mae_holdout:,.2f} ---")
        holdout_results.to_csv(os.path.join(OUTPUT_DIR, "v24_holdout_results.csv"), index=False)
        
        generate_final_report(eval_df, holdout_results, baseline_mae, OUTPUT_DIR)
        
        # --- STAGE 9: V24.0 Intelligence Suite & Dormant Head Manifest ---
        if os.environ.get("WISTERIA_RUN_MODE") == "INTELLIGENCE":
            print("\n--- STAGE 9: Generating V24.0 Intelligence Suite ---")
            arbiter_model = trained_models.get('arbiter_model')
            if arbiter_model:
                importances = arbiter_model.feature_importances_
                feature_names = trained_models['arbiter_feature_names']
                ...
                report_path = os.path.join(OUTPUT_DIR, "v24_arbiter_feature_importance.csv")
                importance_df.to_csv(report_path, index=False)
                print(f"  - Saved XGBoost Arbiter feature importance report to {report_path}")

                # Create the Dormant Head Manifest
                specialist_features = [f for f in feature_names if f.startswith('l0_resid_pred_')]
                dormant_features = importance_df[(importance_df['feature'].isin(specialist_features)) & (importance_df['importance'] == 0)]
                dormant_heads = [f.replace('l0_resid_pred_', '') for f in dormant_features['feature']]
                
                if dormant_heads:
                    dormant_path = os.path.join(OUTPUT_DIR, "dormant_heads.json")
                    with open(dormant_path, 'w') as f:
                        json.dump(dormant_heads, f, indent=2)
                    print(f"  - CREATED DORMANT MANIFEST: Identified {len(dormant_heads)} specialists with zero importance.")
                else:
                    print("  - No dormant heads identified in this run.")
            else:
                print("  - WARNING: Arbiter model not found. Skipping intelligence generation.")
        else:
