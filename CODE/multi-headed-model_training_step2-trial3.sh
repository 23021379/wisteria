...
class FusionModel(nn.Module):
    def __init__(self, feature_configs, msoa_cardinality, head_params, fusion_embed_dim, msoa_embedding_dim=16, fusion_dropout_rate=0.5, use_enhanced_tcn=True, fusion_input_cap=2048, cross_attention_dropout_rate=0.2, attention_dropout_rate=0.1, tcn_dropout=0.3, spatio_temporal_attention_dropout=0.25, stratum_name='monolith'):
        ...
        # --- Set Model Capacity ---
        print(f"  - Building a {'REGULARIZED' if stratum_name != 'monolith' else 'HIGH-CAPACITY'} {stratum_name} model.")
        ...
        # This is the critical fix for the RuntimeError. It calculates the input dimension
        # based on the modules that were actually created, ensuring consistency.
        concatenated_dim = 0
        if self.base_head:
    ...
    def forward(self, x):
        # --- Simplified Forward Pass for Concatenated Model ---
...
def train_one_epoch(model, loader, optimizer, loss_fn, loss_weights={'evidential': 0.5, 'mae': 0.5}):
    model.train()
    total_loss = 0
    for batch in loader:
        batch = {k: v.to(DEVICE) for k, v in batch.items()}
        target = batch['target_price'] # Use a consistent target variable
        ...
        # Use configurable weights for the composite loss
...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for batch in loader:
            batch = {k: v.to(DEVICE) for k, v in batch.items()}
            # The model's forward pass has been simplified and no longer returns intermediates.
            evidential_params, _ = model(batch)
            all_preds.append(evidential_params.cpu())

    # FAIL-SAFE: If the dataloader was empty, return an empty tensor.
    if not all_preds:
        return torch.empty(0, 4) # Return empty tensor with correct final dimension
...
def objective(trial, data_train, y_train, data_val, y_val, feature_configs, msoa_cardinality, y_price_std, y_price_mean, df_val_raw):
    """
    ...
    # The data (`data_train`, `data_val`) is now passed in a pre-transformed state.
    # The defunct call to `preprocess_fold_data` has been removed.
    ...
    trial_params = {
        ...
        'evidential_reg_coeff': trial.suggest_float('evidential_reg_coeff', 0.05, 0.2),
        'undervalue_penalty': trial.suggest_float('undervalue_penalty', 1.1, 1.5),
        # CORRECTED: Add loss weights to the hyperparameter search
    ...
    model = FusionModel(
        ...
        tcn_dropout=trial_params['tcn_dropout'],
    ...
    loss_weights = {
        'evidential': trial_params['evidential_weight'],
        'mae': 1.0 - trial_params['evidential_weight'] # Ensure weights sum to 1
    }

    for epoch in range(EPOCHS):
        # CORRECTED: Pass the tunable loss weights to the training function
        _ = train_one_epoch(model, train_loader, optimizer, loss_fn, loss_weights=loss_weights)
        ...
        # CORRECTED: Use the raw validation target for correct MAPE calculation.
        true_values = df_val_raw['most_recent_sale_price']
...
def generate_final_report(eval_df, holdout_results_df, baseline_mape, output_dir):
    """
    [ROBUST] Generates a comprehensive report with text and plots.
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    # --- FAIL-SAFE: Handle empty or malformed inputs gracefully ---
    ...
    with open(report_path, "w") as f:
        ...
        generalization_gap = (holdout_mape - oof_mape) / (oof_mape + 1e-9)
...
def train_final_model(df_main_transformed, y_main_raw, feature_configs, price_stats, msoa_config, universal_cols_present, LAT_LON_COLS, n_trials):
    """
    [ARCHITECTURALLY ROBUST] Trains the final model on globally pre-transformed data.
    """
    print(f"\n{'='*20} TRAINING FINAL Concatenated-Experts MODEL {'='*20}")
    
    # Unpack config dictionaries for clarity
    y_price_mean = price_stats['y_price_mean']
    y_price_std = price_stats['y_price_std']
    msoa_col_name = msoa_config['column_name']
    msoa_cardinality = msoa_config['cardinality']
    ...
    # CORRECTED: Split the ALREADY TRANSFORMED data to prevent data leakage.
    train_idx_opt, val_idx_opt = train_test_split(df_main_transformed.index, test_size=0.2, random_state=42)
    df_train_opt = df_main_transformed.loc[train_idx_opt]
    df_val_opt = df_main_transformed.loc[val_idx_opt]
    y_train_opt = y_price_for_training.loc[train_idx_opt]
    y_val_opt = y_price_for_training.loc[val_idx_opt]
    
    # The objective function now receives pre-transformed data.
    data_train_optuna, data_val_optuna = {}, {}
    for head_name, config in feature_configs.items():
        cols = config.get('pca_cols') or config.get('raw_cols')
        if cols:
            data_train_optuna[head_name] = df_train_opt[cols]
            data_val_optuna[head_name] = df_val_opt[cols]
    
    data_train_optuna['msoa_id'] = df_train_opt[[msoa_col_name]]
    data_val_optuna['msoa_id'] = df_val_opt[[msoa_col_name]]
    ...
    study.optimize(lambda trial: objective(
        ...
        y_price_std, 
        y_price_mean, 
        y_main_raw.loc[val_idx_opt].to_frame() # Pass raw data as a DataFrame to enforce a robust contract
    ), n_trials=n_trials)
    
    best_params = study.best_params
    print(f"  - Optuna study complete. Best MAPE: {study.best_value:.2f}%")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(df_main_transformed)):
        ...
        df_train_fold = df_main_transformed.iloc[train_idx]
        df_val_fold = df_main_transformed.iloc[val_idx]
        y_train_fold = y_price_for_training.iloc[train_idx]
        y_val_fold = y_price_for_training.iloc[val_idx]
        y_val_raw_fold = y_main_raw.iloc[val_idx] # Get raw target for validation
        
        # --- Data Selection (No Transformation) ---
        data_train_fold, data_val_fold = {}, {}
        for head_name, config in feature_configs.items():
            cols = config.get('pca_cols') or config.get('raw_cols')
            if cols:
                data_train_fold[head_name] = df_train_fold[cols]
                data_val_fold[head_name] = df_val_fold[cols]
        
        data_train_fold['lat_lon'] = df_train_fold[LAT_LON_COLS]
        data_val_fold['lat_lon'] = df_val_fold[LAT_LON_COLS]
        ...
        model = FusionModel(
            ...
            tcn_dropout=best_params.get('tcn_dropout', 0.3), # Use the tuned value
            stratum_name='monolith'
        ).to(DEVICE)
        ...
        # Construct the loss weights from the best hyperparameters found by Optuna.
        loss_weights = {
        ...
        for epoch in range(EPOCHS + 100):
            # Pass the optimized loss weights to the training loop.
            _ = train_one_epoch(model, train_loader, optimizer, loss_fn, loss_weights=loss_weights)
            evidential_preds_val = evaluate(model, val_loader)
            point_preds_scaled = evidential_preds_val.numpy()[:, 0]
            point_preds_unscaled = np.expm1(point_preds_scaled * y_price_std + y_price_mean)
            val_mape = mean_absolute_percentage_error(y_val_raw_fold, point_preds_unscaled)
            scheduler.step(val_mape)
    ...
    # The oof_df needs to be joined with the original raw data to get all columns.
    # The index was reset in the calling function, so we must join on index.
    oof_df = df_main_transformed.join(oof_df)

    # The redundant call to preprocess_fold_data has been removed to enforce state consistency.
    # The canonical artifacts are already saved in main() before this function is called.
...
def predict_on_holdout(df_holdout_raw, best_params, canonical_artifacts, msoa_map, price_stats, output_dir):
    ...
    # CORRECTED: MSOA column exists in the RAW dataframe, not the transformed one.
    ...
    # CORRECTED: Create final results from the raw dataframe
...
def main():
    ...
    HEAD_CONFIG = {
        'head_A_dna': {'type': 'mlp'}, 'head_B_aesthetic': {'type': 'mlp'},
        'head_C_census': {'type': 'mlp'}, 'head_G_gemini_quantitative': {'type': 'mlp'},
        'head_AVM': {'type': 'mlp'}, 'head_compass_raw': {'type': 'tabnet'},
        'head_E_temporal': {'type': 'tcn'},
    }
    N_COMPONENTS_PER_HEAD = {
        "head_A_dna": 100, "head_B_aesthetic": 40, "head_C_census": 100,
        "head_G_gemini_quantitative": 50, "head_AVM": 15, "DEFAULT": 50
    }
    ...
    # Define the lgbm_selector before use to prevent a NameError
