...
class FusionModel(nn.Module):
    def __init__(self, feature_configs, msoa_cardinality, head_params, fusion_embed_dim, msoa_embedding_dim=16, fusion_dropout_rate=0.5, use_enhanced_tcn=True, fusion_input_cap=2048, cross_attention_dropout_rate=0.2, attention_dropout_rate=0.1, tcn_dropout=0.3, spatio_temporal_attention_dropout=0.25, stratum_name='monolith', mode='evidential', tabnet_n_steps=4, tabnet_gamma=1.5):
        ...
        if self.base_head:
            fusion_mlp_input_dim += self.base_head.output_dim
        fusion_mlp_input_dim += msoa_embedding_dim
        if self.specialist_heads:
             fusion_mlp_input_dim += common_output_dim
    ...
    def forward(self, x):
        specialist_outputs = []
        if self.specialist_heads:
            specialist_outputs = [module(x[name]) for name, module in self.specialist_heads.items() if name in x]

        context_vector = None
        if specialist_outputs:
            specialist_tensor = torch.stack(specialist_outputs, dim=1)
            context_vector, _ = self.hierarchical_attention(specialist_tensor)
        
        base_output = self.base_head(x['head_base']) if self.base_head and 'head_base' in x else None
        msoa_embed = self.msoa_embedding(x['msoa_id'])

        final_concat_list = []
        if base_output is not None:
            final_concat_list.append(base_output)
        final_concat_list.append(msoa_embed)
        if context_vector is not None:
            final_concat_list.append(context_vector)
...
class ShapModelWrapper(nn.Module):
    ...
    def forward(self, *tensors):
        ...
        # Ensure msoa_id is the correct type (long) for the embedding layer
        if 'msoa_id' in input_dict:
            input_dict['msoa_id'] = input_dict['msoa_id'].long()
...
def objective(trial, data_train, y_train, data_val, y_val, feature_configs, msoa_cardinality, y_price_std, y_price_mean, df_val_raw, mode='evidential', base_params=None):
    ...
    if mode == 'point_prediction':
        trial_params = {
            ...
            # Add the missing head dimension parameters
            'fusion_embed_dim': trial.suggest_categorical('fusion_embed_dim', [64, 128]),
            'large_head_hidden_dim': trial.suggest_categorical('large_head_hidden_dim', [128, 256]),
            ...
            'tabnet_n_steps': trial.suggest_int('tabnet_n_steps', 3, 6),
...
def generate_shap_reports_for_holdout(df_main_raw, df_holdout_raw, best_params, canonical_artifacts, msoa_map, msoa_col_name, holdout_results_df, output_dir):
    ...
    # Use the final feature_configs derived from the canonical artifacts
    _, feature_configs = transform_with_canonical_artifacts(background_data.head(1), canonical_artifacts)
    best_head_params = create_head_params(best_params, feature_configs)
    ...
    # --- CORRECTED: Prepare background data and pass ALL inputs as tensors ---
    bg_data_processed = preprocess_for_inference(background_data, canonical_artifacts)
    bg_data_processed['msoa_id'] = pd.DataFrame(
    ...
    ordered_keys = sorted(list(bg_data_processed.keys()))
    background_tensor_list = [
        torch.tensor(bg_data_processed[k].values, dtype=torch.float32).to(DEVICE) for k in ordered_keys
    ]
    ...
    for idx, property_row in tqdm(df_holdout_raw.iterrows(), total=len(df_holdout_raw)):
        ...
        property_data_processed = preprocess_for_inference(property_df, canonical_artifacts)
        property_data_processed['msoa_id'] = pd.DataFrame(
        ...
        explain_tensor_list = [
            torch.tensor(property_data_processed[k].values, dtype=torch.float32).to(DEVICE) for k in ordered_keys
        ]
        
        shap_values_list = explainer.shap_values(explain_tensor_list)
        
        # Reconstruct full feature names from the ordered, processed data
        full_feature_names = [col for key in ordered_keys for col in property_data_processed[key].columns]
...
echo "all done! The model training artifacts are available at ${OUTPUT_GCS_DIR}."m