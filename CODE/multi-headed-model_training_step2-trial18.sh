...
pip install pandas pyarrow gcsfs google-cloud-storage scikit-learn lightgbm fuzzywuzzy optuna matplotlib seaborn python-Levenshtein tqdm shap xgboost hdbscan tabulate umap-learn
...
def main():
    ...
    # --- [A.D-63.0] CANONICAL METADATA QUARANTINE PROTOCOL (V65 HARDENED) ---
    # This is the single source of truth for all columns that are NOT features.
    # Every feature selection or modeling process MUST filter against this list.
    CANONICAL_METADATA_COLS = [
        ...
        # High-Leakage Transactional History (Quarantined from selection)
        'price_change_since_last',

        # Geographic & Stratification Flags
        'postcode_area', 'error_zone_flag',

        # Triage Gate Metadata (V65)
        'data_completeness_score', # Renamed from data_credibility_score
        'is_data_deficient',
    ...
    df_rightmove_raw = pd.read_csv(os.environ.get("RIGHTMOVE_DATA_LOCAL_PATH"), header=None, names=['address_info', 'sales_history'], on_bad_lines='skip')
    ...
    df['canonical_lookup_key'] = _create_canonical_lookup_key(df['property_id'])
    ...
    # Define columns that are METADATA and should NOT be converted to numeric features.
    # This list is now the single source of truth for non-feature columns at this stage.
    meta_cols_to_exclude = {
        'property_id', 'normalized_address_key', 'rightmove_address_text', 'pcds',
        'postcode_area', 'error_zone_flag', 'most_recent_sale_price', 
        'rightmove_row_id', 'address_info',
        'is_data_deficient', 'data_completeness_score',
        'score_intrinsic_dna', 'score_transactional_context', 'score_qualitative_context'
    }
    
    feature_cols = [c for c in df_final.columns if c not in meta_cols_to_exclude]
    ...
    # --- [A.D-V69.1] STAGE 7: THE V69 HYDRA ARCHITECTURE LOCK-IN ---
    v69_model_package = train_v69_architecture(
        df_main_raw, feature_sets, universal_cols, 
        full_feature_sets_for_diagnostics, CANONICAL_METADATA_COLS, OUTPUT_DIR
    )
    joblib.dump(v69_model_package, os.path.join(OUTPUT_DIR, "v69_model_package.joblib"))

    print("\n--- STAGE 8: Final Validation & Reporting (V69) ---")
    if not df_holdout_raw.empty:
        holdout_results = predict_on_holdout_v69(df_holdout_raw, v69_model_package)
        ...
        print(f"--- HOLDOUT SET FINAL RESULTS (V69.1): MAE £{final_mae_holdout:,.2f} ---")
        holdout_results.to_csv(os.path.join(OUTPUT_DIR, "v69_holdout_results.csv"), index=False)
        
        print("\n--- STAGE 9: Generating V69 Diagnostic Omnibus Report ---")
        # generate_v69_diagnostic_omnibus_report(holdout_results, df_holdout_raw, v69_model_package, OUTPUT_DIR)

    print("\n V62.0 Pipeline finished successfully.")
...
# [A.D-V67.0] CANONICAL RECONCILIATION KEY GENERATOR
def _create_canonical_lookup_key(address_series):
    """
    Creates a robust, normalized lookup key from a raw address string series.
    It lowercases, removes all non-alphanumeric characters, and strips whitespace.
    This ensures consistent matching between disparate data sources.
    """
    print("  - Generating canonical lookup keys for data reconciliation...")
    # Ensure input is string, handle potential NaNs gracefully
    s = address_series.astype(str).str.lower()
    # Remove all characters that are not lowercase letters or numbers
...
def train_v69_architecture(df_main_raw, feature_sets, universal_cols, full_feature_sets_for_diagnostics, non_feature_cols, output_dir):
    """
    [V69.1] Orchestrates the Hydra protocol, training the Generalist, Anomaly Corrector,
    and a standalone Impoverished Predictor with an empirical feature contract.
    """
    print("\n--- [V69.1] Engaging Hydra Protocol Training Orchestrator ---")

    # --- 1. Pillar I: Train Generalist & Sentinel on Purified Data ---
    ...
    if len(df_generalist_candidates) > 100:
        print("\n--- [V69.0] Pillar I: Training Generalist & Sentinel ---")
        generalist_model, _ = train_v18_residual_ensemble(
            df_generalist_candidates, feature_sets, universal_cols, output_dir, is_sub_model=True, domain_name="v69_generalist"
        )
        sentinel_detector = train_sentinel_detector(df_generalist_candidates, universal_cols)
    else:
        print("  - WARNING: Insufficient data for Generalist/Sentinel training.")

    # --- 2. Three-Way Data Partitioning ---
    df_impoverished_train = df_main_raw[df_main_raw['is_data_deficient'] == 1].copy()
    df_anomaly_train, df_generalist_train = pd.DataFrame(), pd.DataFrame() # Init
    if sentinel_detector is not None:
        print("\n--- [V69.0] Applying Sentinel Gate for final data partitioning... ---")
        ...
        print(f"  - Partition complete: {len(df_generalist_train)} Generalist, {len(df_anomaly_train)} Anomaly, {len(df_impoverished_train)} Impoverished.")
    else:
        df_generalist_train = df_generalist_candidates # Fallback if Sentinel failed

    # --- 3. Pillar II: Train the Anomaly Corrector ---
    ...
    if generalist_model and not df_anomaly_train.empty:
        ...
        # Circuit Breaker from V68.2 is implicitly handled by the hardened corrector function
    
    # --- 4. Pillar III: Train the Impoverished Predictor with Empirical Contract ---
    impoverished_feature_contract = []
    try:
        print("\n--- [V69.1] Generating Empirical Feature Contract for Impoverished Predictor ---")
        MIN_PREVALENCE_THRESHOLD, TOP_N_FEATURES = 0.50, 100
        v67_importance_path = os.path.join(output_dir, "v67_residual_analysis", "archetype_-1", "feature_importance.csv")
        availability_path = os.path.join(output_dir, "quarantine_data_availability_report.csv")
        
        v67_importance_df = pd.read_csv(v67_importance_path)
        availability_df = pd.read_csv(availability_path)
        
        merged_intelligence = pd.merge(v67_importance_df, availability_df, left_on='feature', right_on='feature_name')
        reliable_features_df = merged_intelligence[merged_intelligence['prevalence_quarantine'] >= MIN_PREVALENCE_THRESHOLD]
        print(f"  - Found {len(reliable_features_df)} features with >{MIN_PREVALENCE_THRESHOLD:.0%} prevalence.")
        
        final_contract_df = reliable_features_df.sort_values('importance', ascending=False).head(TOP_N_FEATURES)
        impoverished_feature_contract = final_contract_df['feature_name'].tolist()
        assert impoverished_feature_contract, "Empirical contract generation resulted in an empty list."
        print(f"  - Finalized Empirical Contract with {len(impoverished_feature_contract)} features.")
    except Exception as e:
        print(f"!!! [ WARNING ] Could not generate Empirical Feature Contract. Error: {e} !!!")
        print("!!! FALLING BACK TO STATIC SANCTUARY LIST. PERFORMANCE MAY BE DEGRADED.    !!!")
        impoverished_feature_contract = [c for c in df_impoverished_train.columns if c.startswith(('ae_', 'persona_', 'yearly_'))]
        impoverished_feature_contract += ['latitude', 'longitude', 'data_completeness_score']
        impoverished_feature_contract = sorted(list(set([c for c in impoverished_feature_contract if c in df_impoverished_train.columns])))

    impoverished_predictor_package = train_impoverished_predictor(df_impoverished_train, impoverished_feature_contract)

    # --- 5. Package the Final Artifact ---
    v69_model_package = {
        ...
        'impoverished_predictor_model': impoverished_predictor_package,
        'contracts': {'universal_cols': universal_cols},
        'architecture_version': 'V69.1_Hydra_Empirical'
    }
    return v69_model_package

def predict_on_holdout_v69(df_holdout_raw, v69_model_package):
    """[V69.1] Implements the Hydra three-path inference protocol."""
    print("\n--- [V69.1] Engaging Hydra Three-Path Inference Router ---")
    if df_holdout_raw.empty: return pd.DataFrame()

    generalist = v69_model_package['generalist_model']
    sentinel = v69_model_package['sentinel_anomaly_detector']
    corrector = v69_model_package['anomaly_corrector_model']
    predictor = v69_model_package['impoverished_predictor_model']
    universal_cols = v69_model_package['contracts']['universal_cols']
    
    results = df_holdout_raw[['property_id', 'most_recent_sale_price', 'is_data_deficient', 'data_completeness_score']].copy()
    ...
    if sentinel is not None and generalist_candidates_mask.any():
        X_sentinel = df_holdout_raw[generalist_candidates_mask][universal_cols].fillna(0)
        anomaly_preds = sentinel.predict(X_sentinel)
    ...
    # --- PATH 1: PREDICTOR_IMPOVERISHED ---
    if impoverished_mask.any() and predictor:
        print(f"  - Routing {impoverished_mask.sum()} properties to Impoverished Predictor...")
        X_pred = df_holdout_raw[impoverished_mask][predictor['feature_contract']].fillna(0)
        preds_log = predictor['model'].predict(X_pred)
        results.loc[impoverished_mask, 'predicted_price'] = np.expm1(preds_log)
        results.loc[impoverished_mask, 'routing_decision'] = 'PREDICTOR_IMPOVERISHED'

    # --- PATH 2: CORRECTOR_ANOMALY ---
    if anomaly_mask.any() and corrector:
    ...
    # Handle fallbacks for any unpredicted rows (e.g., if a model was disabled)
    ...
    if unpredicted_mask.any():
        ...
        results.loc[unpredicted_mask, 'routing_decision'] += '_DEFAULT'
        
    return results

def generate_v68_diagnostic_omnibus_report(holdout_results_df, df_holdout_raw, v68_model_package, output_dir):
    """[V68.1 HARDENED] Generates the diagnostic report for the Cerberus Airlock architecture."""
    print("\n--- [V68.1] Generating Hardened Diagnostic Omnibus Report ---")
    report_path = os.path.join(output_dir, "v68_diagnostic_omnibus.md")
    
    results = holdout_results_df.copy()
    # Ensure MAPE calculation is robust to zero sale price
    results['mape'] = (results['absolute_error'] / results['most_recent_sale_price'].replace(0, np.nan)).abs()
    
    with open(report_path, "w") as f:
        f.write(f"# Wisteria V68.1: Cerberus Airlock Diagnostic Report\n\n_Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}_\n\n")
        f.write("## 1. Outlier Projection Analysis (Hypothesis Confirmation)\n\n")
        f.write("This plot projects the V67 Generalist failures into the V67 UMAP space. Their proximity to the Quarantine Cohort confirms the out-of-distribution failure mode.\n\n")
        f.write("![Outlier Projection Analysis](v68_outlier_projection_analysis.png)\n\n")
        
        f.write("## 2. Cerberus Triage Performance Analysis\n\n")
        triage_perf = results.groupby('routing_decision').agg(
            Count=('property_id', 'count'),
            Mean_MAE=('absolute_error', 'mean'),
            Mean_MAPE=('mape', 'mean')
        ).sort_values('Mean_MAPE', ascending=False)
        f.write(triage_perf.to_markdown(floatfmt=",.2f") + "\n\n")
        
        f.write("## 3. Outlier Containment Analysis (Zero-Tolerance Verdict)\n\n")
        outliers = results.sort_values('mape', ascending=False).head(15)
        f.write(outliers[['property_id', 'most_recent_sale_price', 'predicted_price', 'mape', 'routing_decision', 'data_completeness_score']].to_markdown(index=False, floatfmt=".2%") + "\n\n")
        
        # --- A.D-V68.1-R3 AUTOMATED VERDICT MANDATE ---
        generalist_failures = outliers[outliers['routing_decision'] == 'GENERALIST']
        if generalist_failures.empty:
            f.write("**VERDICT: SUCCESS. ZERO `GENERALIST` FAILURES DETECTED IN TOP 15 OUTLIERS.**\n\n")
        else:
            f.write("**VERDICT: CATASTROPHIC FAILURE. `GENERALIST` OUTLIERS DETECTED IN TOP 15. The Sentinel Gate has been breached.**\n\n")
        # --- END MANDATE ---
        
        f.write("## 4. Unified Corrector Intelligence\n\n")
        corrector_package = v68_model_package.get('corrector_model')
        if corrector_package:
            model = corrector_package['model']
            features = corrector_package['feature_contract']
            importance_df = pd.DataFrame({'Feature': features, 'Importance': model.feature_importances_}).sort_values('Importance', ascending=False)
            f.write("Top 30 features influencing the unified error corrector model:\n\n")
            f.write(importance_df.head(30).to_markdown(index=False) + "\n\n")
        else:
            f.write("_Unified Corrector model was not trained._\n\n")
...
def train_impoverished_predictor(df_impoverished_train, feature_contract):
    """
    [V69.1 Hardened] Trains a standalone predictor on the data-starved
    cohort using a dynamically generated, empirical feature contract.
    """
    print("\n--- [V69.0] Pillar III: Training the Impoverished Predictor (Standalone) ---")
    if df_impoverished_train.empty or not feature_contract:
        print("  - No impoverished data or feature contract. Predictor will be disabled.")
        return None

    y_impoverished = np.log1p(df_impoverished_train['most_recent_sale_price'])
    X_impoverished = df_impoverished_train[feature_contract].copy().fillna(0)

    predictor_model = lgb.LGBMRegressor(
        n_estimators=750, learning_rate=0.02, num_leaves=15,
        random_state=42, n_jobs=-1
    )
    predictor_model.fit(X_impoverished, y_impoverished)
    
    print("  - Impoverished Predictor training complete.")
