...
echo "Updating system packages and installing Python venv tooling..."
sudo apt-get update
# ARCHITECTURAL MANDATE: Install the python3-venv package to ensure the
# virtual environment can be created successfully on minimal VM images.
sudo apt-get install -y libgomp1 python3-venv
# [ARCHITECTURALLY REMOVED] NVIDIA drivers are not required for the CPU-based architecture.
...
def train_final_model(df_train_raw, df_tune_raw, feature_sets, specialist_config, universal_cols_present):
    ...
    """
    # --- PHASE 1: Generate Specialist OOF Predictions & LEAVES for BOTH Train and Tune sets ---
    print("\n--- PHASE 1: Generating Specialist OOF Predictions & Leaf Indices ---")
    ...
    all_oof_leaves_dfs = [] # Store one-hot encoded leaf DFs for each head
    ...
    for head_name in specialist_heads_to_train:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf_specialist.split(X_specialist)):
            ...
            # 1. Get standard predictions (still valuable)
            oof_preds_for_head[val_idx] = model.predict(X_specialist.iloc[val_idx])
            
            # 2. Get leaf indices as latent representation
            leaf_indices = model.predict(X_specialist.iloc[val_idx], pred_leaf=True)
            leaf_df_fragment = pd.DataFrame(leaf_indices, index=X_specialist.iloc[val_idx].index)
            oof_leaves_fragments_for_head.append(leaf_df_fragment)

        oof_specialist_preds_df[f'oof_pred_{head_name}'] = oof_preds_for_head

        # Combine, prefix, and one-hot encode the leaves for this head
        head_leaves_full_df = pd.concat(oof_leaves_fragments_for_head).sort_index()
        head_leaves_full_df = head_leaves_full_df.add_prefix(f'leaf_{head_name}_tree_')
        head_leaves_one_hot = pd.get_dummies(head_leaves_full_df, columns=head_leaves_full_df.columns, sparse=True, dtype=np.uint8)
        all_oof_leaves_dfs.append(head_leaves_one_hot)
    ...
    # Combine all one-hot encoded leaf representations
    oof_specialist_leaves_combined_df = pd.concat(all_oof_leaves_dfs, axis=1)
    
    X_fusion_main_aug = pd.concat([
        df_main_raw[all_original_features].fillna(0),
        oof_specialist_preds_df,
        interaction_features_df,
        oof_specialist_leaves_combined_df
    ], axis=1)
    ...
    study.optimize(objective_single, n_trials=30)
...
def generate_shap_reports_for_holdout(df_main_raw, df_holdout_raw, trained_models, feature_sets, universal_cols_present, holdout_results_df, output_dir):
    """
    [ARCHITECTURALLY REBUILT] Generates SHAP explanations for the two-stage LGBM ensemble.
    """
    import shap
    print("\n--- Generating SHAP Explanations for 'Council of Experts' Model ---")
    ...
    # This requires running stage 1 of the prediction pipeline on both background and holdout data.
    specialist_heads = [k.replace('specialist_', '') for k in trained_models if k.startswith('specialist_')]
    fusion_model = trained_models['fusion_model'] # Needed for reindexing
    ...
    def get_augmented_features(df):
        ...
        for head_name in specialist_heads:
            ...
            # Get leaf indices
            leaf_indices = model.predict(X_specialist, pred_leaf=True)
            head_leaves_df = pd.DataFrame(leaf_indices, index=df.index).add_prefix(f'leaf_{head_name}_tree_')
            head_leaves_one_hot = pd.get_dummies(head_leaves_df, columns=head_leaves_df.columns, sparse=True, dtype=np.uint8)
            all_leaves_dfs.append(head_leaves_one_hot)
        ...
        leaves_combined_df = pd.concat(all_leaves_dfs, axis=1)

        X_augmented_raw = pd.concat([
            df[all_original_features].fillna(0),
            specialist_preds_df,
            interaction_features_df,
            leaves_combined_df
        ], axis=1)
        
        # ARCHITECTURAL CONTRACT: Align columns with the trained model's feature set.
        training_cols = fusion_model.feature_name_
        return X_augmented_raw.reindex(columns=training_cols, fill_value=0)
    ...
    X_background_augmented = get_augmented_features(background_data)

    print("  - Preparing holdout data for SHAP explanation...")
    X_holdout_augmented = get_augmented_features(df_holdout_raw)

    # --- 2. Initialize and run the TreeExplainer ---
    print("  - Initializing SHAP TreeExplainer...")
    explainer = shap.TreeExplainer(fusion_model, X_background_augmented)
    
    print(f"  - Calculating SHAP values for {len(X_holdout_augmented)} holdout properties...")
    shap_values = explainer.shap_values(X_holdout_augmented)

    # --- 3. Format and save the report ---
    shap_df = pd.DataFrame(shap_values, columns=X_holdout_augmented.columns, index=X_holdout_augmented.index)
    
    # Add key info for the report
    report_df = holdout_results_df.copy()
    report_df['shap_base_value'] = explainer.expected_value
    
    # Get top 5 contributors for each property
    top_contributors = []
...
def predict_on_holdout(df_holdout_raw, trained_models, feature_sets, universal_cols_present):
    """
    [ARCHITECTURALLY REBUILT] Generates predictions on the holdout set using the
    two-stage "Council of Experts" ensemble.
    """
    print("\n--- STAGE 10: Final Evaluation on True Holdout Set ---")
    if df_holdout_raw.empty:
        return pd.DataFrame()

    # --- STAGE 1: Generate predictions and leaf indices from specialist models ---
    print("  - Generating predictions and leaf indices from specialist models...")
    holdout_specialist_preds_df = pd.DataFrame(index=df_holdout_raw.index)
    all_holdout_leaves_dfs = []
    specialist_heads = [k.replace('specialist_', '') for k in trained_models if k.startswith('specialist_')]
    ...
    for head_name in specialist_heads:
        ...
        # Get leaf indices
        leaf_indices = model.predict(X_holdout_specialist, pred_leaf=True)
        head_leaves_df = pd.DataFrame(leaf_indices, index=df_holdout_raw.index).add_prefix(f'leaf_{head_name}_tree_')
        head_leaves_one_hot = pd.get_dummies(head_leaves_df, columns=head_leaves_df.columns, sparse=True, dtype=np.uint8)
        all_holdout_leaves_dfs.append(head_leaves_one_hot)

    # --- STAGE 2: Generate final predictions from the fusion model ---
    print("  - Assembling augmented holdout feature set...")
    all_original_features = [
    ...
    holdout_specialist_leaves_combined_df = pd.concat(all_holdout_leaves_dfs, axis=1)

    X_fusion_holdout_raw = pd.concat([
        df_holdout_raw[all_original_features].fillna(0),
        holdout_specialist_preds_df,
        interaction_features_holdout_df,
        holdout_specialist_leaves_combined_df
    ], axis=1)
    ...
    # ARCHITECTURAL CONTRACT: Align columns with the trained model's feature set.
    training_cols = fusion_model.feature_name_
    X_fusion_holdout = X_fusion_holdout_raw.reindex(columns=training_cols, fill_value=0)

    final_preds_log = fusion_model.predict(X_fusion_holdout)
...
def main():
    ...
    trained_models, eval_df = train_final_model(
    ...
    # Save the master model artifact
    ...
    # CORRECTED: The new `eval_df` from train_final_model already contains the final, unscaled prediction.
    ...
    print(f"\n--- OOF PERFORMANCE (COUNCIL OF LGBM EXPERTS) ---")
