...
# --- [A.D-V77.2] The Bifurcated Infusion Contract (Dynamically Generated) ---
    ...
    # --- [A.D-V79.0] STAGE 7: THE V79 "PRAGMATIC OPTIMUM" ARCHITECTURE LOCK-IN ---
    v79_model_package = train_v79_architecture(
    ...
    joblib.dump(v79_model_package, os.path.join(OUTPUT_DIR, "v79_model_package.joblib"))

    print("\n--- STAGE 8: Final Validation & Reporting (V79) ---")
    if not df_holdout_raw.empty:
        holdout_results = predict_on_holdout_v79(df_holdout_raw, v79_model_package, imputation_map)
        ...
        print(f"--- HOLDOUT SET FINAL RESULTS (V79.0): MAE £{final_mae_holdout:,.2f} ---")
        holdout_results.to_csv(os.path.join(OUTPUT_DIR, "v79_holdout_results.csv"), index=False)
        
        print("\n--- STAGE 9: Generating V79 Diagnostic Omnibus Report ---")
        generate_v79_diagnostic_omnibus_report(holdout_results, df_holdout_raw, v79_model_package, OUTPUT_DIR)
...
def train_v79_architecture(df_main_raw, feature_sets, universal_cols, full_feature_sets_for_diagnostics, non_feature_cols, output_dir, imputation_map, CONTEXTUAL_INFUSION_COLS, FINANCIAL_INFUSION_COLS):
    """
    [A.D-V79.0] Orchestrates the "Pragmatic Optimum" protocol with a battle-hardened DAE.
    """
    print("\n--- [V79.0] Engaging Pragmatic Optimum Protocol ---")
    ...
    if len(df_generalist_candidates) > 100:
        generalist_model, _ = train_v18_residual_ensemble(
            df_generalist_candidates, feature_sets, universal_cols, output_dir, is_sub_model=True, domain_name="v74_generalist"
        )
    ...
    # --- 3. [V78] Execute the three pillars of hyperparameter optimization ---
    dae_package, dae_study_results = None, None
    ...
    # A.D-V80.1: The Canonical Embedding Feature Set
    # This stable, curated list is the NON-NEGOTIABLE input for the DAE.
    # It is engineered to capture the intrinsic, physical reality of a property.
    # Version: 1.0
    CANONICAL_EMBEDDING_FEATURES = [
        # === Group 1: Core Physical DNA (The "Blueprint") ===
        # --- Size & Volume ---
        'num__floor_area_sqm_from_homipi__numeric_or_empty__hm',
        'num__number_of_bedrooms_from_homipi__numeric_or_empty__hm',
        'num__number_of_reception_rooms_from_homipi__numeric_or_empty__hm',
        'num__plot_size_sqm_or_info_text__numeric_extracted_or_original_text__bnl',
        'num_rooms_identified_in_step5',
        'other_OtherBedrooms_count',
        'primary_MainLivingArea_area_sqm',
        'primary_MainKitchen_area_sqm', # Added for core layout
        'primary_MainBathroom_area_sqm', # Added for core layout

        # --- Archetype & Era ---
        'num__property_construction_year_from_homipi__YYYY_or_2025NewBuildOrError__hm',
        'num__property_main_type_encoded__1Flat_hm',
        'num__property_sub_type_code_from_homipi__4Detached_hm',
        'num__property_sub_type_code_from_homipi__5Semi_Detached_hm',
        'num__property_sub_type_code_from_homipi__6Terraced_hm',
        'num__property_tenure_code_from_homipi__2Freehold_hm', # '1LeaseholdOrOther' is implied

        # === Group 2: Core Geographic Identity (The "Location") ===
        'latitude',
        'longitude',
        'pcd_latitude', # Postcode district level, provides regional context
        'pcd_longitude',# Postcode district level, provides regional context
        'pcd_eastings', # Raw grid reference, can capture different patterns
        'pcd_northings',# Raw grid reference, can capture different patterns
        'geo_cluster',  # K-Means cluster provides a simple market grouping

        # === Group 3: Core Visual & Quality Identity (The "Condition" - Our Proprietary Edge) ===
        'avg_persona_rating_overall',
        'std_dev_persona_rating_overall',
        'num_images_total',
        'primary_MainKitchen_renovation_score',
        'primary_MainBathroom_renovation_score',
        'primary_MainExteriorFront_renovation_score',
        'primary_MainLivingArea_renovation_score', # Added for core living space quality
        '_renovation_score', # The global property renovation score
        '_num_flaws',      # The global property flaw count
        'Composite_CoreLiving_Strength', # A derived score of living space quality
        'Opportunity_Good_Bones_Score',  # A derived score of potential

        # === Group 4: Core Locational Context (The "Immediate Environment" - Stable & Non-Financial) ===
        'LSOA_Property_Density',
        'num__train_station_2_distance__numeric_extracted_or_original_text__bnl',
        'num__primary_or_first_type_school_1_ofsted_rating_encoded__numeric_extracted_or_original_text__bnl',
        'MSOA11_CLASSIFICATION_NAME_Hard_pressed_flat_dwellers', # Example of a stable census archetype feature
        'OA11_CLASSIFICATION_NAME_Detached_retirement_living',     # Example of a stable census archetype feature
    ]

    if not df_generalist_train.empty:
        # Call the new static, hardened DAE trainer
        dae_package = train_generalist_dae(df_generalist_train, CANONICAL_EMBEDDING_FEATURES)
    ...
    # --- 4. Package the Final Artifact (V78 Observability Contract) ---
    v79_model_package = {
        ...
        'study_results': { # DAE results are now static, only Chimera is dynamic
            'chimera_predictor': chimera_study_results
        },
        'architecture_version': 'V79.0_Pragmatic_Optimum'
    }
    return v79_model_package

def predict_on_holdout_v74(df_holdout_raw, v74_model_package, imputation_map):
    """[A.D-V74.0] Implements the DAE-Enhanced Targeted Infusion three-path inference protocol."""
    print("\n--- [V74.0] Engaging DAE-Enhanced Three-Path Inference Router ---")
    if df_holdout_raw.empty: return pd.DataFrame()

    generalist = v74_model_package['generalist_model']
    sentinel = v74_model_package['sentinel_anomaly_detector']
    corrector = v74_model_package['anomaly_corrector_model']
    chimera_predictor = v74_model_package['chimera_predictor_model']
    dae_package = v74_model_package['dae_package']
    df_library = v74_model_package['df_generalist_train_library']
    contracts = v74_model_package['contracts']
    ...
    results['donor_council_map'] = None # For V74 diagnostics
    ...
    # --- PATH 1: CHIMERA PREDICTOR (IMPOVERISHED VIA TARGETED INFUSION) ---
    if impoverished_mask.any() and chimera_predictor and dae_package and not df_library.empty:
        ...
        # Symmetrically find donors using the DAE-enhanced function and perform infusion
        donor_council_map = find_dae_enhanced_donor_council(df_impoverished_holdout, df_library, dae_package)
        df_healed_holdout = perform_targeted_infusion(df_impoverished_holdout, df_library, donor_council_map, imputation_map)
        ...
        # Store donor map for diagnostics
        # Convert map from {property_id: council} to a Series for easy mapping
    ...
    # Handle fallbacks for any unpredicted rows
...
def generate_v79_diagnostic_omnibus_report(holdout_results, df_holdout_raw, v79_model_package, output_dir):
    """[A.D-V79.0] Generates the diagnostic report for the Pragmatic Optimum architecture."""
    print("\n--- [V79.0] Generating V79 Diagnostic Omnibus Report ---")
    report_path = os.path.join(output_dir, "v79_diagnostic_omnibus.md")
    
    study_results = v79_model_package.get('study_results', {})
    chimera_results = study_results.get('chimera_predictor')

    with open(report_path, "w") as f:
        f.write(f"# Wisteria V79.0: Pragmatic Optimum Diagnostic Report\n\n")
        f.write("## 1. Chimera Predictor Tuning Summary (Pillar III)\n\n")
        if chimera_results:
        ...
        f.write("## 2. Dynamic Alpha Analysis (Pillar II)\n\n")
        ...
        f.write("## 3. V79 Donor Council Dissection (High-Error Cases)\n\n")
        chimera_errors = chimera_results_df.sort_values('absolute_error', ascending=False).head(10)
        if chimera_errors.empty:
            f.write("_No high-error Chimera cases to dissect._\n\n")
        else:
            for _, row in chimera_errors.iterrows():
                council_info = row['donor_council_map']
                f.write(f"<hr>\n\n**Property ID:** `{row['property_id']}` (MAE: `£{row['absolute_error']:,.0f}`)\n\n")
                if isinstance(council_info, dict):
                    f.write(f"- **Winning Alpha:** `{council_info.get('best_alpha')}`\n")
                    f.write(f"- **Final Coherence Score:** `{council_info.get('coherence_score'):.4f}`\n\n")
                else:
                    f.write("_Donor council metadata not available._\n\n")
    print(f"  - V79 Diagnostic Omnibus Report saved to {report_path}")
...
def train_generalist_dae(df_generalist, feature_subset, noise_factor=0.2):
    """
    [A.D-V77.1 COMPLIANT] Trains the DAE with a performance-gated, robust protocol.
    """
    print("\n--- [V77.1] Pillar I: Training DAE with Performance-Gated Protocol ---")
    
    # --- V79 Configuration (Non-Negotiable) ---
    V79_PARAMS = {
        'n_layers': 2, 'latent_dim': 128, 'layer_size_decay_factor': 0.7837695730418882,
        'learning_rate': 0.005524433540578556, 'dropout_rate': 0.11127942234243929
    }

    # Data Prep
    X_clean_raw = df_generalist[feature_subset].fillna(0)
    ...
    # Build static model from V79 params
    encoder, decoder = build_dae_from_params(n_features, V79_PARAMS)
    model = GeneralistDAE(n_features, V79_PARAMS['latent_dim']).to(DEVICE)
    model.encoder, model.decoder = encoder, decoder

    # Train with V77 robust mechanics (extended epochs, early stopping, LR scheduler)
    best_val_loss, _ = train_dae_for_one_trial(model, X_train_scaled, X_val_scaled, V79_PARAMS['learning_rate'], final_training=True)

    # --- [A.D-V77.1] Non-Negotiable Performance Gate ---
    if best_val_loss >= 0.4:
        raise ValueError(
            f"DAE training failed performance gate. Final validation loss {best_val_loss:.4f} was not below 0.40."
        )
    print(f"  - V77 DAE Performance Gate PASSED (Final Loss: {best_val_loss:.4f}).")

    model.encoder.eval()
    return {'encoder': model.encoder, 'scaler': scaler, 'feature_subset': feature_subset}
...
def perform_targeted_infusion(df_impoverished, df_reference, donor_council_map, imputation_map, CONTEXTUAL_INFUSION_COLS, FINANCIAL_INFUSION_COLS):
    ...
    return df_healed_infused_value
