...
# --- ARCHITECTURAL BLUEPRINT: Head Processing Configuration (INERT IN PHASE 1) ---
# This dictionary will become the single source of truth for defining how each
# feature head is preprocessed and modeled. In this phase, it is defined but not
# yet used by any part of the pipeline.
HEAD_CONFIG = {
    # TabNet Heads (High-dimensionality tabular data)
    'head_A_dna_raw': {'type': 'tabnet'},
    'head_C_census': {'type': 'tabnet'},
    'head_compass_raw': {'type': 'tabnet'},

    # MLP Heads (Lower-dimensionality or pre-engineered features)
    'head_A_dna_microscope': {'type': 'mlp'},
    'head_A_dna_engineered': {'type': 'mlp'},
    'head_B_aesthetic': {'type': 'mlp'},
    'head_AVM': {'type': 'mlp'},
    
    # Pre-Embedded Heads (Features from offline autoencoders, requires only scaling)
    'head_G_gemini_quantitative': {'type': 'pre_embedded'},

    # TCN Heads (Temporal sequence data)
    'head_E_temporal': {'type': 'tcn'},
    # Any 'head_F_spatio_temporal_*' heads will also be treated as 'tcn' by future logic
}
...
def create_head_params(params_dict, feature_configs):
    ...
    LARGE_HEADS = [
        'head_A_dna_raw', 'head_A_dna_engineered', 'head_C_census', 
        'head_compass_raw', 'head_E_temporal'
    ]
...
class EnhancedTCNHead(nn.Module):
    def __init__(self, input_dim, n_features_per_timestep, n_timesteps, num_channels, kernel_size=2, dropout=0.2, attention_hidden_dim=64):
...
class FusionModel(nn.Module):
    def __init__(self, feature_configs, msoa_cardinality, head_params, fusion_embed_dim, msoa_embedding_dim=16, fusion_dropout_rate=0.5, use_enhanced_tcn=True, fusion_input_cap=2048, cross_attention_dropout_rate=0.2, attention_dropout_rate=0.1, tcn_dropout=0.3, spatio_temporal_attention_dropout=0.25, stratum_name='monolith', mode='evidential', tabnet_n_steps=4, tabnet_gamma=1.5, fusion_mlp_depth='deep'):
        ...
        if fusion_mlp_depth == 'deep':
            ...
            print("  - Using DEEP final fusion MLP: [1024, 512, 256]")
        else: # shallow
            fusion_mlp_dims = [512, 256]
            print("  - Using SHALLOW final fusion MLP: [512, 256]")
        ...
        for head_name, config in feature_configs.items():
            ...
            # ARCHITECTURAL FIX: Add all heads, including 'head_base', to the unified dictionary.
            self.heads[head_name] = head_module
        
        self.hierarchical_attention = HierarchicalAttention(embedding_dim=common_output_dim)
        
        # ARCHITECTURAL FIX: The MLP input is now simple and robust: the single context
        # vector from the unified attention mechanism plus the MSOA embedding.
        fusion_mlp_input_dim = common_output_dim + msoa_embedding_dim

        self.msoa_embedding = nn.Embedding(msoa_cardinality, msoa_embedding_dim)
        print(f"--- UNIFIED Hierarchical Fusion: Final MLP input dimension: {fusion_mlp_input_dim} ---")
    ...
    def forward(self, x):
        head_outputs = [module(x[name]) for name, module in self.heads.items() if name in x]

        # ARCHITECTURAL FIX: If there are no feature inputs, we cannot make a valid prediction.
        if not head_outputs:
            # Return a zero tensor of the correct shape to avoid crashing downstream.
            batch_size = next(iter(x.values())).shape[0]
            msoa_embed_device = self.msoa_embedding.weight.device
            dummy_output = torch.zeros(batch_size, 1, device=msoa_embed_device)
            if self.mode == 'point_prediction':
                return dummy_output
            else:
                return torch.zeros(batch_size, 4, device=msoa_embed_device), dummy_output

        # If we have head outputs, proceed with the full, correct fusion logic.
        all_heads_tensor = torch.stack(head_outputs, dim=1)
        global_context_vector, _ = self.hierarchical_attention(all_heads_tensor)
        
        # Ensure msoa_id is the correct shape and type for the embedding layer.
        msoa_embed = self.msoa_embedding(x['msoa_id'].long().squeeze(-1))

        # The MSOA embedding is now correctly and safely concatenated with the
        # context vector it is intended to modify. This is guaranteed to have the
        # correct shape for the fusion_mlp.
        final_fused = torch.cat([global_context_vector, msoa_embed], dim=1)
...
def fit_and_create_canonical_artifacts(df, feature_sets, n_components_map, universal_cols_present, head_config, trained_autoencoders):
    ...
    pre-fitted objects (Autoencoders, Scalers) for ALL head types. Runs ONCE.
    """
    print("\n--- STAGE 4.5: Fitting/Assembling Unified Global Preprocessors ---")
    canonical_artifacts = {}
    
    # Process MLP heads with Autoencoders
    for head_name, cols in feature_sets.items():
        ...
        if head_name in trained_autoencoders:
            ...
            canonical_artifacts[head_name] = {
                ...
                'scaler': ae_artifact['scaler'], # The scaler used during AE training is required for inference
                'original_cols': available_cols,
                'input_dim': ae_artifact['latent_dim']
            }
        else:
            # Fallback to PCA for any MLP heads not designated for AE processing
            ...
            print(f"  - [MLP/PCA] Using PCA fallback for '{head_name}': {len(available_cols)} -> {n_comp} components.")
            canonical_artifacts[head_name] = {
                'type': 'pca', 'scaler': scaler, 'pca': pca,
                'original_cols': available_cols, 'input_dim': n_comp
            }
    ...
    for head_name, cols in feature_sets.items():
        ...
        # --- TCN-Specific Pre-validation to ensure artifact consistency ---
        if head_type == 'tcn':
            # ARCHITECTURAL FIX: First, identify the subset of columns that are actually parsable.
            # This prevents a state mismatch where the scaler is fit on more columns than the
            # TCN structure accounts for.
            spatio_pattern = re.compile(r"^(.*)_(\d{4})_(.*)$")
            temporal_pattern = re.compile(r"^(\d{4})_(.*)$")
            
            # This becomes the single source of truth for this TCN head.
            valid_tcn_cols = [
                col for col in available_cols 
                if spatio_pattern.match(col) or temporal_pattern.match(col)
            ]
            
            if not valid_tcn_cols:
                print(f"  - WARNING: No parsable TCN columns found for '{head_name}'. Skipping.")
                continue

            print(f"  - [TCN] Validating features for '{head_name}': {len(available_cols)} initial -> {len(valid_tcn_cols)} parsable.")
            cols_to_process = valid_tcn_cols
        else: # For TabNet, all available columns are valid.
            cols_to_process = available_cols

        print(f"  - [{head_type.upper()}] Fitting Scaler for '{head_name}' on {len(cols_to_process)} features...")
        head_df_raw = df[cols_to_process].copy().apply(pd.to_numeric, errors='coerce').fillna(0)
        ...
        artifact = {
            'type': head_type, 
            'scaler': scaler, 
            'raw_cols': cols_to_process,
            'input_dim': len(cols_to_process)
        }

        if head_type == 'tcn':
            # Now, parse the GUARANTEED valid columns to get TCN dimensions.
            parsed_cols = {}
            spatio_pattern = re.compile(r"^(.*)_(\d{4})_(.*)$")
            temporal_pattern = re.compile(r"^(\d{4})_(.*)$")
            for col in cols_to_process: # Use the clean list
                year, feature_stem = None, None
                match_spatio = spatio_pattern.match(col)
                match_temporal = temporal_pattern.match(col)
                if match_spatio:
...
def transform_with_canonical_artifacts(df_raw, artifacts, msoa_map=None):
    """
    [ARCHITECTURALLY HARDENED] Transforms a raw dataframe into a single, UNIFIED,
    model-ready dataframe containing both transformed features and essential non-feature
    columns, guaranteeing data-label-MSOA alignment.
    """
    ...
    for head_name, config in artifacts.items():
        if 'original_cols' in config or 'raw_cols' in config:
            ...
            available_cols = [col for col in expected_cols if col in df_raw.columns]
            missing_cols = set(expected_cols) - set(available_cols)
            head_df_raw = df_raw[available_cols].copy().apply(pd.to_numeric, errors='coerce').fillna(0)
            if missing_cols:
                for col in missing_cols: head_df_raw[col] = 0
            ...
            if config['type'] == 'pca':
                ...
                df_transformed = pd.concat([df_transformed, pd.DataFrame(head_pca_result, columns=pca_cols, index=df_raw.index)], axis=1)
                feature_configs[head_name] = {'input_dim': head_pca_result.shape[1], 'raw_cols': pca_cols}
                ...
                df_transformed = pd.concat([df_transformed, pd.DataFrame(latent_vector.numpy(), columns=ae_cols, index=df_raw.index)], axis=1)
                ...
                if config['type'] == 'tabnet': head_feature_config['is_tabnet'] = True
                if config['type'] == 'tcn':
    ...
    # --- NEW MANDATE: UNIFY DATAFRAME POST-TRANSFORMATION ---
    passthrough_cols = ['most_recent_sale_price', 'property_id']
    if msoa_map and msoa_map.get('column_name') in df_raw.columns:
        passthrough_cols.append(msoa_map['column_name'])
        
    cols_to_join = [col for col in passthrough_cols if col in df_raw.columns]
    df_unified = df_transformed.join(df_raw[cols_to_join], how='inner')
    
    if msoa_map:
        msoa_col_name = msoa_map['column_name']
        df_unified[msoa_col_name] = df_unified[msoa_col_name].map(msoa_map['codes']).fillna(msoa_map['unknown_id']).astype(int)

    return df_unified, feature_configs
...
def validate_merge_quality(df, sample_size=10):
    ...
    def normalize_for_comparison1(s):
    ...
    for i, (_, row) in enumerate(sample_df.iterrows()):
        ...
        similarity_score = fuzz.token_set_ratio(
            normalize_for_comparison1(feature_addr),
            normalize_for_comparison1(rightmove_addr)
        )
...
def objective(trial, data_train, y_train, data_val, y_val, feature_configs, msoa_cardinality, y_price_std, y_price_mean, df_val_raw, mode='evidential', base_params=None):
    ...
    if mode == 'point_prediction':
        trial_params = {
            ...
            'fusion_embed_dim': trial.suggest_categorical('fusion_embed_dim', [64, 96, 128]),
            'large_head_hidden_dim': trial.suggest_categorical('large_head_hidden_dim', [128, 192, 256]),
            'small_head_hidden_dim': trial.suggest_categorical('small_head_hidden_dim', [64, 128]),
            ...
            'tabnet_n_steps': trial.suggest_int('tabnet_n_steps', 3, 8),
            'tabnet_gamma': trial.suggest_float('tabnet_gamma', 1.2, 1.8),
...
def train_final_model(df_main_transformed, y_main_raw, feature_configs, price_stats, msoa_config, universal_cols_present, LAT_LON_COLS, n_trials):
    ...
    # Return the paths to the saved models to ensure a hardened inference pipeline
    saved_model_paths = [os.path.join(OUTPUT_DIR, f"model_fold_{f}.pt") for f in range(NUM_FOLDS_FINAL)]
    
    return oof_df, best_params, saved_model_paths, feature_configs
...
def generate_shap_reports_for_holdout(df_main_raw, df_holdout_raw, best_params, canonical_artifacts, msoa_map, msoa_col_name, holdout_results_df, output_dir, feature_configs):
    """
    [SYSTEMICALLY HARDENED] Generates structured SHAP explanations using the Frozen Head
    Approximation, with added resilience against sparse data and implementation bugs.
    """
    import shap
    print("\n--- Generating Structured SHAP Explanations for Holdout Set (Hardened Protocol) ---")
    if df_holdout_raw.empty:
        print("  - Holdout set is empty. Skipping SHAP report generation.")
        return

    # --- 1. Load the FULL, original model to use as a pre-processor. ---
    best_head_params = create_head_params(best_params, feature_configs)
    ...
    full_model = FusionModel(
        ...
        fusion_embed_dim=best_params['fusion_embed_dim'], msoa_embedding_dim=best_params['msoa_embedding_dim'],
        fusion_dropout_rate=best_params['fusion_dropout_rate'], use_enhanced_tcn=True,
        tcn_dropout=best_params.get('tcn_dropout', 0.3), tabnet_n_steps=best_params.get('tabnet_n_steps', 4), 
        tabnet_gamma=best_params.get('tabnet_gamma', 1.5), fusion_mlp_depth=best_params.get('fusion_mlp_depth', 'deep')
    ).to(DEVICE)
    full_model.load_state_dict(torch.load(model_path))
    full_model.eval()

    tabnet_head_names = [name for name, head in full_model.heads.items() if isinstance(head, TabNetHead)]
    ordered_keys_for_shap = sorted(list(feature_configs.keys()))

    # --- 2. Define the temporary, SHAP-COMPATIBLE model for the fusion part. ---
    class ShapAgnosticFusionPart(nn.Module):
        def __init__(self, original_model):
            super().__init__()
            self.compatible_heads = nn.ModuleDict({ name: head for name, head in original_model.heads.items() if not isinstance(head, TabNetHead) })
            self.msoa_embedding = original_model.msoa_embedding
            self.hierarchical_attention = original_model.hierarchical_attention
            self.fusion_mlp = original_model.fusion_mlp
            self.evidential_head = original_model.evidential_head

        def forward(self, *inputs):
            input_dict = dict(zip(ordered_keys_for_shap, inputs))
            compatible_outputs = [ self.compatible_heads[name](input_dict[name]) for name in self.compatible_heads if name in input_dict and input_dict[name] is not None]
            frozen_tabnet_outputs = [input_dict[name] for name in tabnet_head_names if name in input_dict and input_dict[name] is not None]
            
            all_head_outputs = compatible_outputs + frozen_tabnet_outputs
            if not all_head_outputs: return torch.zeros(1, 1, device=self.msoa_embedding.weight.device)

            all_heads_tensor = torch.stack(all_head_outputs, dim=1)
            context_vector, _ = self.hierarchical_attention(all_heads_tensor)
            msoa_embed = self.msoa_embedding(input_dict['msoa_id'].long().squeeze(-1))
            final_fused = torch.cat([context_vector, msoa_embed], dim=1)
            final_representation = self.fusion_mlp(final_fused)
            evidential_output = self.evidential_head(final_representation)
            return evidential_output[:, 0:1]

    # --- 3. Prepare background data with resilience against missing features. ---
    background_data_raw = df_main_raw.sample(min(100, len(df_main_raw)), random_state=42)
    bg_data_transformed, _ = transform_with_canonical_artifacts(background_data_raw, canonical_artifacts)
    
    data_for_model_bg = {}
    for head_name, config in feature_configs.items():
        cols = config.get('raw_cols')
        if cols:
            available_cols = [c for c in cols if c in bg_data_transformed.columns]
            if available_cols:
                data_for_model_bg[head_name] = bg_data_transformed[available_cols]
            else:
                print(f"  - SHAP WARNING: No features for '{head_name}' in background data. Using zero placeholder.")
                num_rows, num_features = len(bg_data_transformed), config['input_dim']
                data_for_model_bg[head_name] = pd.DataFrame(np.zeros((num_rows, num_features)), columns=cols, index=bg_data_transformed.index)

    data_for_model_bg['msoa_id'] = pd.DataFrame(background_data_raw[msoa_col_name].map(msoa_map['codes']).fillna(msoa_map['unknown_id']).astype(int))
    
    background_data_tensors = {k: torch.tensor(v.values, dtype=torch.float32).to(DEVICE) for k, v in data_for_model_bg.items()}
    with torch.no_grad():
        frozen_bg_embeddings = {name: full_model.heads[name](background_data_tensors[name]).detach() for name in tabnet_head_names}

    # --- 4. Create HYBRID background tensor list with DEFINITIVE conditional logic. ---
    background_tensor_list_hybrid = []
    for key in ordered_keys_for_shap:
        if key in frozen_bg_embeddings:
            background_tensor_list_hybrid.append(frozen_bg_embeddings[key])
        else:
            background_tensor_list_hybrid.append(background_data_tensors.get(key))

    # --- 5. Initialize the explainer. ---
    explanation_model = ShapAgnosticFusionPart(full_model).to(DEVICE)
    explainer = shap.DeepExplainer(explanation_model, background_tensor_list_hybrid)
    print("  - DeepExplainer initialized successfully with Hardened Protocol.")

    # --- 6. Generate explanations in a loop. ---
    all_shap_reports = []
    for idx, property_row in tqdm(df_holdout_raw.iterrows(), total=len(df_holdout_raw)):
        # (Logic for preparing per-property data and generating hybrid tensor list is repeated here)
        property_df_raw = pd.DataFrame([property_row])
        property_df_transformed, _ = transform_with_canonical_artifacts(property_df_raw, canonical_artifacts)
        data_for_model_prop = { head: property_df_transformed[conf['raw_cols']] for head, conf in feature_configs.items() if 'raw_cols' in conf }
        data_for_model_prop['msoa_id'] = pd.DataFrame(property_df_raw[msoa_col_name].map(msoa_map['codes']).fillna(msoa_map['unknown_id']).astype(int))
        prop_tensors = {k: torch.tensor(v.values, dtype=torch.float32).to(DEVICE) for k, v in data_for_model_prop.items()}
        with torch.no_grad():
            frozen_prop_embeddings = {name: full_model.heads[name](prop_tensors[name]).detach() for name in tabnet_head_names}
        
        explain_tensor_list_hybrid = []
        for key in ordered_keys_for_shap:
            if key in frozen_prop_embeddings:
                explain_tensor_list_hybrid.append(frozen_prop_embeddings[key])
            else:
                explain_tensor_list_hybrid.append(prop_tensors.get(key))
        
        shap_values_list = explainer.shap_values(explain_tensor_list_hybrid)
        
        # --- 7. Reconstruct the report from the hybrid SHAP values. ---
        shap_df_rows = []
        for i, key in enumerate(ordered_keys_for_shap):
            if shap_values_list[i] is None: continue
            shap_values_for_input = shap_values_list[i].flatten()
            if key in tabnet_head_names:
                shap_df_rows.append({'feature': f"EMBEDDING_HEAD::{key}", 'shap_value': shap_values_for_input.sum(), 'head': key})
            elif key == 'msoa_id':
                 shap_df_rows.append({'feature': 'msoa_embedding', 'shap_value': shap_values_for_input.sum(), 'head': key})
            else:
                cols = feature_configs[key]['raw_cols']
                for j, col_name in enumerate(cols):
                    shap_df_rows.append({'feature': col_name, 'shap_value': shap_values_for_input[j], 'head': key})

        shap_df = pd.DataFrame(shap_df_rows)
        head_level_shap = shap_df.groupby('head')['shap_value'].sum().to_dict()
        top_5_contributors = shap_df.reindex(shap_df['shap_value'].abs().sort_values(ascending=False).index).head(5)

        property_info = holdout_results_df.loc[idx]
        report = {
            'holdout_index': idx, 'property_id': property_info['property_id'], 'predicted_price': property_info['predicted_price'],
            'actual_price': property_info['most_recent_sale_price'], 'absolute_error': property_info['absolute_error'],
            'epistemic_uncertainty': property_info['epistemic_uncertainty'], 'shap_base_value': explainer.expected_value[0],
...
def predict_on_holdout(df_holdout_raw, best_params, canonical_artifacts, msoa_map, price_stats, saved_model_paths, feature_configs):
    """
    [DEFINITIVE & HARDENED] Generates predictions using a unified data source,
    eliminating the data-feature misalignment bug.
    """
    if df_holdout_raw.empty or not saved_model_paths:
        print("  - Holdout set is empty or no model paths provided. Skipping prediction.")
        return pd.DataFrame()

    print("  - [UNIFIED PIPELINE] Creating model-ready holdout data...")
    # The single call to the hardened transformer. It returns ONE unified dataframe.
    df_holdout_unified, _ = transform_with_canonical_artifacts(df_holdout_raw, canonical_artifacts, msoa_map)
    
    # --- ALL DATA IS NOW SOURCED FROM THE SINGLE UNIFIED DATAFRAME ---
    data_for_model = {}
    for head_name, config in feature_configs.items():
        cols = config.get('raw_cols')
        if cols:
            data_for_model[head_name] = df_holdout_unified[cols]
    
    msoa_col_name = msoa_map['column_name']
    data_for_model['msoa_id'] = df_holdout_unified[[msoa_col_name]]

    y_price_log_holdout = np.log1p(df_holdout_unified['most_recent_sale_price'])
    y_price_scaled_holdout = pd.Series((y_price_log_holdout - price_stats['y_price_mean']) / price_stats['y_price_std'], index=df_holdout_unified.index)
    
    dataset = WisteriaDataset(data_for_model, y_price_scaled_holdout, y_price_scaled_holdout)
    loader = DataLoader(dataset, batch_size=2048, shuffle=False, num_workers=2, pin_memory=True)

    # --- Hardened Inference Protocol (Model Loading is Unchanged) ---
    head_params = create_head_params(best_params, feature_configs)
    model = FusionModel(
        feature_configs=feature_configs, msoa_cardinality=msoa_map['unknown_id'] + 1, head_params=head_params,
        fusion_embed_dim=best_params['fusion_embed_dim'], msoa_embedding_dim=best_params['msoa_embedding_dim'],
        fusion_dropout_rate=best_params['fusion_dropout_rate'], use_enhanced_tcn=True,
        tcn_dropout=best_params.get('tcn_dropout', 0.3), stratum_name='monolith', mode='evidential',
    ...
    all_fold_preds = []
    print("  - Performing inference with hardened stateful model loading...")
    for model_path in saved_model_paths:
        if not os.path.exists(model_path):
            print(f"    - WARNING: Model file not found at {model_path}. Skipping this fold.")
            continue
        print(f"    - Loading model state from: {model_path}")
        model.load_state_dict(torch.load(model_path))
        preds = evaluate(model, loader, mode='evidential')
        all_fold_preds.append(preds)
    
    del model; gc.collect(); torch.cuda.empty_cache()

    if not all_fold_preds: 
        print("  - FATAL: No fold models were loaded for holdout prediction.")
        return pd.DataFrame()
    ...
    results_df = df_holdout_unified[['property_id', 'most_recent_sale_price']].copy()
...
def main():
    ...
    def normalize_for_comparison(s):
        """
        [ARCHITECTURALLY HARDENED] Canonicalizes an address string by lowercasing, 
        removing non-alphanumerics, and correctly handling stringified list inputs.
        """
        raw_str = str(s)
        
        # ARCHITECTURAL FIX: Proactively and safely handle stringified list format,
        # which is the root cause of the corrupted merge similarity scores.
        if raw_str.startswith('[') and raw_str.endswith(']'):
            try:
                ...
                # If parsing fails for any reason, fall back to using the raw string.
                pass
                
        # Proceed with the original, correct normalization logic.
    ...
    # --- HYBRID ARCHITECTURE: STAGE 5.5 - Generate LGBM Price Prior Feature ---
    print("\n--- STAGE 5.5: Generating LGBM Price Prior Feature (Leakage-Proof) ---")
    # First, determine the optimal feature set for the LGBM model, using the main data only.
    lgbm_feature_selector = lgb.LGBMRegressor(random_state=42)
    all_numeric_features = [col for col in df_main_raw.columns if col not in ['most_recent_sale_price', 'property_id', 'normalized_address_key', 'final_merge_key', 'address'] and pd.api.types.is_numeric_dtype(df_main_raw[col])]
    lgbm_feature_selector.fit(df_main_raw[all_numeric_features].fillna(0), np.log1p(df_main_raw['most_recent_sale_price']))
    importances = pd.DataFrame({'feature': all_numeric_features, 'importance': lgbm_feature_selector.feature_importances_}).sort_values('importance', ascending=False)
    N_TOP_GLOBAL_FEATURES = 750
    top_global_features = importances.head(N_TOP_GLOBAL_FEATURES)['feature'].tolist()

    # <<< ARCHITECTURAL MANDATE: PURGE DATA LEAKAGE >>>
    print("\n--- Sanitizing LGBM feature set to purge potential data leaks ---")
    leaky_keywords = [
        'sale_price', 'sold_price', # Obvious target leaks
        'sale_year', 'sale_month', 'sold_date', # Date components of the target event
        'homipi_estimated_price', # High-risk AVMs that might be too close to the target
    ]
    
    initial_feature_count = len(top_global_features)
    sanitized_features = []
    removed_leaks = []
    ...
    for feature in top_global_features:
        ...
        for keyword in leaky_keywords:
            if keyword in feature.lower():
                is_leaky = True
                removed_leaks.append(feature)
                break
        if not is_leaky:
            sanitized_features.append(feature)
    
    if removed_leaks:
        print(f"  - Purged {len(removed_leaks)} potential leak(s) from LGBM feature set.")
        print(f"  - Sample purged features: {removed_leaks[:5]}")

    # The rest of the LGBM logic MUST use the sanitized list.
    top_global_features = sanitized_features
    print(f"  - Final sanitized LGBM feature count: {len(top_global_features)}")
    # <<< END OF LEAKAGE PURGE BLOCK >>>

    print(f"  - Selected top {len(top_global_features)} features for the LGBM feature generator.")

    # Train the feature generator model ON THE MAIN DATA ONLY.
    lgbm_feature_generator = lgb.LGBMRegressor(
        random_state=42, n_estimators=2000, learning_rate=0.02, num_leaves=31,
        n_jobs=-1, colsample_bytree=0.7, subsample=0.7, reg_alpha=0.1, reg_lambda=0.1
    )
    print("  - Training LGBM feature generator on main data...")
    lgbm_feature_generator.fit(
        df_main_raw[top_global_features].fillna(0),
        np.log1p(df_main_raw['most_recent_sale_price'])
    )
    
    # Generate the feature for both training and holdout sets.
    # This is a leakage-proof operation as the model has never seen the holdout labels.
    main_prior_log = lgbm_feature_generator.predict(df_main_raw[top_global_features].fillna(0))
    df_main_raw['lgbm_price_prior'] = main_prior_log
    
    holdout_prior_log = lgbm_feature_generator.predict(df_holdout_raw[top_global_features].fillna(0))
    df_holdout_raw['lgbm_price_prior'] = holdout_prior_log
    print("  - Successfully generated and added 'lgbm_price_prior' to both dataframes.")

    # --- HYBRID ARCHITECTURE: Add new head for LGBM prior ---
    print("\n--- Integrating LGBM prior as a new expert head ---")
    feature_sets['head_LGBM_prior'] = ['lgbm_price_prior']
    HEAD_CONFIG['head_LGBM_prior'] = {'type': 'mlp'}
    print("  - Registered 'head_LGBM_prior' in model configuration.")
    ...
    # 2. Prepare MSOA mapping and target variable statistics from the raw training data.
    # This must be done BEFORE transformation to ensure the map is based on the original codes.
    ...
    _, msoa_uniques = pd.factorize(df_main_raw[msoa_col].fillna('Unknown'))
    ...
    # 3. Create the single, unified, model-ready training dataframe.
    # This function now handles all transformations AND the MSOA integer encoding.
    print("  - [UNIFIED PIPELINE] Creating model-ready training data...")
    df_main_unified, final_feature_configs = transform_with_canonical_artifacts(
        df_main_raw, 
        canonical_artifacts, 
        msoa_map
    )
    ...
    # <<< ARCHITECTURAL MANDATE: VALIDATE BASELINE ON TRUE HOLDOUT SET >>>
    print("\n--- VALIDATING BASELINE on True Holdout Set to check for overfitting ---")
    if not df_holdout_raw.empty:
        X_holdout_baseline = df_holdout_raw[top_global_features].fillna(0)
        y_holdout_baseline = df_holdout_raw['most_recent_sale_price']

        # Use the model from the last fold for a representative performance check
        holdout_preds_log = lgbm_baseline.predict(X_holdout_baseline)
        holdout_preds_unscaled = np.expm1(holdout_preds_log)
        
        true_baseline_mae_holdout = mean_absolute_error(y_holdout_baseline, holdout_preds_unscaled)
        print(f"--- LIGHTGBM TRUE HOLDOUT MAE: £{true_baseline_mae_holdout:,.2f} ---")
        # This value is now our definitive benchmark.
    ...
    eval_df, best_params, saved_model_paths, authoritative_feature_configs = train_final_model(
    ...
    if not df_holdout_raw.empty:
        ...
        holdout_results = predict_on_holdout(
            ...
            msoa_map,
            price_stats,
            saved_model_paths, # Pass the exact artifacts to the inference function
            authoritative_feature_configs # Pass the authoritative config
        )
        ...
        generate_shap_reports_for_holdout(
            ...
            feature_configs=authoritative_feature_configs # Pass the authoritative config
        )
