...
# RE-ARCHITECTED V29.0 ORCHESTRATOR
def train_regional_arbiter_ensemble_v29(df_main_raw, initial_feature_sets, NON_FEATURE_COLS, output_dir):
    """
    [V29.0 ARCHITECTURE]
    Orchestrates the Arbiter ensemble with fully DECOUPLED and ASYMMETRIC
    refinement and feature selection pipelines for Generalist and Specialist models.
    """
    print(f"\n--- V29.0 ASYMMETRIC ARBITER TRAINING INITIATED ---")
    
    # --- Step 1: Data Partitioning ---
    print("  - Stage 1: Partitioning data into General and Specialist zones...")
    is_error_zone = df_main_raw['error_zone_flag'] == 1
    df_specialist_train = df_main_raw[is_error_zone].copy()
    df_general_train = df_main_raw[~is_error_zone].copy()
    assert len(df_specialist_train) > 0, "FATAL: Data partitioning failed. Specialist zone has zero samples."
    assert len(df_general_train) > 0, "FATAL: Data partitioning failed. General zone has zero samples."

    # --- Step 2: Independent Pipeline for the GENERALIST ---
    print("\n--- [DOMAIN: GENERALIST] Kicking off independent refinement and training pipeline ---")
    generalist_feature_sets = initial_feature_sets.copy()
    if os.environ.get("REFINEMENT_ENABLED") == "true":
        generalist_refinement_dir = os.path.join(os.environ.get("REFINEMENT_DATA_LOCAL_DIR"), "generalist")
        if os.path.exists(generalist_refinement_dir):
            print("  - Generalist refinement intelligence found. Engaging auto-refinement...")
            generalist_feature_sets = perform_auto_refinement(initial_feature_sets, generalist_refinement_dir, int(os.environ.get("HEAD_CULLING_IMPORTANCE_THRESHOLD")), int(os.environ.get("FEATURE_CULLING_IMPORTANCE_THRESHOLD")))
        else:
            print("  - WARNING: Generalist refinement intelligence not found. Skipping auto-refinement.")

    generalist_all_candidates = [c for c in df_general_train.columns if c not in NON_FEATURE_COLS]
    generalist_universal_cols = select_features_with_lgbm(df_general_train, df_general_train['most_recent_sale_price'], generalist_all_candidates, 250, "Generalist Core")
    
    generalist_models, _ = train_v18_residual_ensemble(
        df_general_train, generalist_feature_sets, generalist_universal_cols, output_dir, is_sub_model=True, domain_name="generalist"
    )
    
    # --- Step 3: Independent Pipeline for the SPECIALIST ---
    print("\n--- [DOMAIN: SPECIALIST] Kicking off independent refinement and training pipeline ---")
    specialist_feature_sets = initial_feature_sets.copy()
    if os.environ.get("REFINEMENT_ENABLED") == "true":
        specialist_refinement_dir = os.path.join(os.environ.get("REFINEMENT_DATA_LOCAL_DIR"), "specialist")
        if os.path.exists(specialist_refinement_dir):
            print("  - Specialist refinement intelligence found. Engaging auto-refinement...")
            specialist_feature_sets = perform_auto_refinement(initial_feature_sets, specialist_refinement_dir, int(os.environ.get("HEAD_CULLING_IMPORTANCE_THRESHOLD")), int(os.environ.get("FEATURE_CULLING_IMPORTANCE_THRESHOLD")))
        else:
            print("  - WARNING: Specialist refinement intelligence not found. Skipping auto-refinement.")

    specialist_all_candidates = [c for c in df_specialist_train.columns if c not in NON_FEATURE_COLS]
    specialist_universal_cols = select_features_with_lgbm(df_specialist_train, df_specialist_train['most_recent_sale_price'], specialist_all_candidates, 150, "Specialist Core")
    
    specialist_models, _ = train_v18_residual_ensemble(
        df_specialist_train, specialist_feature_sets, specialist_universal_cols, output_dir, is_sub_model=True, domain_name="specialist"
    )

    # --- Step 4: Arbiter Training ---
    print("\n  - Stage 4: Generating Out-of-Fold predictions for Arbiter training...")
    # CRITICAL: Augment the model artifacts with their domain-specific feature lists before prediction.
    generalist_models['universal_cols'] = generalist_universal_cols
    generalist_models['feature_sets'] = generalist_feature_sets
    specialist_models['universal_cols'] = specialist_universal_cols
    specialist_models['feature_sets'] = specialist_feature_sets

    generalist_oof_preds = predict_on_holdout_v18(df_main_raw, generalist_models, return_oof_log_preds=True)
    specialist_oof_preds = predict_on_holdout_v18(df_main_raw, specialist_models, return_oof_log_preds=True)
    
    print("\n  - Stage 5: Training the Arbiter...")
    X_arbiter = pd.DataFrame({
        'generalist_pred_log': generalist_oof_preds, 'specialist_pred_log': specialist_oof_preds,
        'is_error_zone_flag': is_error_zone.astype(int),
        'latitude': df_main_raw['latitude'].fillna(df_main_raw['latitude'].mean()),
        'longitude': df_main_raw['longitude'].fillna(df_main_raw['longitude'].mean())
    })
    y_main_log = np.log1p(df_main_raw['most_recent_sale_price'])
    arbiter_params = {'n_estimators': 500, 'learning_rate': 0.02, 'num_leaves': 15, 'random_state': 42, 'n_jobs': -1, 'verbosity': -1}
    final_arbiter_model = lgb.LGBMRegressor(**arbiter_params).fit(X_arbiter, y_main_log)

    # --- Step 6: Assemble Final Model Artifact and OOF Predictions ---
    trained_models = {
        'generalist_ensemble': generalist_models, 'specialist_ensemble': specialist_models,
        'arbiter_model': final_arbiter_model, 'architecture_version': 'V29.0_Asymmetric_Arbiter'
    }
    eval_df = df_main_raw[['property_id', 'most_recent_sale_price']].copy()
    eval_df['final_predicted_price'] = np.expm1(final_arbiter_model.predict(X_arbiter))
    
    _generate_and_save_lgbm_diagnostics(final_arbiter_model, X_arbiter, "Arbiter", os.path.join(output_dir, "v29_diagnostics", "arbiter"))
    
    return trained_models, eval_df

# NEW V29.0 ASYMMETRIC PREDICTION ORCHESTRATOR
# DEFINITIVE V29.0 ASYMMETRIC PREDICTION ORCHESTRATOR
def predict_on_holdout_v29(df_holdout_raw, trained_models):
    """
    [V29.0 ARCHITECTURE] Predicts using the fully decoupled Asymmetric Arbiter ensemble.
    This version correctly reconstructs the necessary state for the Arbiter.
    """
    print("\n--- V29.0: Predicting with Asymmetric Regional Arbiter Ensemble ---")
    if df_holdout_raw.empty: return pd.DataFrame()

    # --- Step 1: Unpack the complete model artifact ---
    generalist_models = trained_models['generalist_ensemble']
    specialist_models = trained_models['specialist_ensemble']
    arbiter_model = trained_models['arbiter_model']

    # --- Step 2: Get predictions from the domain-specific sub-models ---
    generalist_preds_log = predict_on_holdout_v18(df_holdout_raw, generalist_models, return_oof_log_preds=True)
    specialist_preds_log = predict_on_holdout_v18(df_holdout_raw, specialist_models, return_oof_log_preds=True)
    
    # --- Step 3: [CORRECTED] Prepare features for the Arbiter by RECONSTRUCTING state ---
    # The holdout dataframe does not have the 'error_zone_flag'. We must create it here,
    # using the globally-defined, authoritative helper function.
    df_holdout_raw['postcode_area'] = get_postcode_area(df_holdout_raw['property_id'], df_holdout_raw.get('pcds'))
    is_error_zone = df_holdout_raw['postcode_area'].isin(HIGH_ERROR_POSTCODE_AREAS)
    
    X_arbiter_holdout = pd.DataFrame({
        'generalist_pred_log': generalist_preds_log,
        'specialist_pred_log': specialist_preds_log,
        'is_error_zone_flag': is_error_zone.astype(int),
        'latitude': df_holdout_raw['latitude'].fillna(df_holdout_raw['latitude'].mean()),
        'longitude': df_holdout_raw['longitude'].fillna(df_holdout_raw['longitude'].mean())
    })
    
    # --- Step 4: Make the final prediction using the Arbiter ---
...
# NEW V27.0 PREDICTION FUNCTION
def predict_on_holdout_v27(df_holdout_raw, trained_models):
    print("\n--- V27.0: Predicting with Regional Arbiter Ensemble ---")
    if df_holdout_raw.empty: return pd.DataFrame()

    # [A.D-27.2 CORRECTION] The V18 predictor is self-contained; no need to unpack args here.

    # Get predictions from the Generalist
    generalist_preds_log = predict_on_holdout_v18(df_holdout_raw, trained_models['generalist_ensemble'], return_oof_log_preds=True)
    
    # Get predictions from the Specialist
    specialist_preds_log = predict_on_holdout_v18(df_holdout_raw, trained_models['specialist_ensemble'], return_oof_log_preds=True)
    
    # Prepare features for the Arbiter
    is_error_zone = df_holdout_raw['error_zone_flag'] == 1
    ...
    # Make the final prediction using the Arbiter
    final_preds_log = trained_models['arbiter_model'].predict(X_arbiter_holdout)
    final_preds = np.expm1(final_preds_log)
...
def main():
    ...
    # --- [A.D-27.2] STAGE 5: STRATIFIED Sanitization & Splitting ---
    print("\n--- STAGE 5: Stratified Sanitization & Splitting ---")
    
    # Step 5.1: Create the stratification key BEFORE the split.
    print("  - Generating stratification key ('error_zone_flag')...")
    df['postcode_area'] = get_postcode_area(df['property_id'], df.get('pcds')) # Uses the globally-scoped helper
    df['error_zone_flag'] = df['postcode_area'].isin(HIGH_ERROR_POSTCODE_AREAS).astype(int)
    stratify_key = df['error_zone_flag']

    # Step 5.2: Sanitize the feature matrix (as before)
    df_model_data = df.drop(columns=[c for c in QUARANTINED_KEYS + ['postcode_area', 'error_zone_flag'] if c in df.columns], errors='ignore')
    for col in df_model_data.select_dtypes(include=['object', 'category']).columns:
        if col not in QUARANTINED_KEYS:
            df_model_data[col] = pd.factorize(df_model_data[col])[0]

    # Step 5.3: Recombine and perform the STRATIFIED split
    ...
    # The stratification key must be aligned with the dataframe being split.
    stratify_key_aligned = stratify_key.loc[df_final.index]

    print(f"  - Performing stratified split. Error zone prevalence: {stratify_key_aligned.mean():.2%}")
    df_main_raw, df_holdout_raw = train_test_split(
        df_final,
        test_size=0.15,
        random_state=42,
        stratify=stratify_key_aligned # THE CRITICAL CORRECTION
    )


    # --- V17 PRE-FLIGHT FEATURE SELECTION ---
    # STAGE 6 is now obsolete and has been moved inside the V29 orchestrator.

    print("\n--- STAGE 7: A.D-V29.0 Asymmetric Arbiter Protocol Training ---")
    
    # Define non-feature columns here to be passed down.
    NON_FEATURE_COLS = QUARANTINED_KEYS + ['postcode_area', 'error_zone_flag', 'most_recent_sale_price']
    
    trained_models, eval_df = train_regional_arbiter_ensemble_v29(
        df_main_raw, feature_sets, NON_FEATURE_COLS, OUTPUT_DIR
    )
    joblib.dump(trained_models, os.path.join(OUTPUT_DIR, "v29_regional_arbiter_models.joblib"))
    ...
    print(f"--- OOF PERFORMANCE (V29.0 ASSEMBLY): MAE £{final_mae_oof:,.2f} ---")
    eval_df.to_csv(os.path.join(OUTPUT_DIR, "v29_oof_predictions.csv"), index=False)
    
    if not df_holdout_raw.empty:
        # [A.D-29.0] Call the new, correct V29 prediction orchestrator
        holdout_results = predict_on_holdout_v29(df_holdout_raw, trained_models)
        ...
        print(f"--- HOLDOUT SET FINAL RESULTS (V29.0): MAE £{final_mae_holdout:,.2f} ---")
        holdout_results.to_csv(os.path.join(OUTPUT_DIR, "v29_holdout_results.csv"), index=False)
        
        # The old report requires a baseline_mae which is no longer calculated in this flow.
        # generate_final_report(eval_df, holdout_results, baseline_mae, OUTPUT_DIR)
        print("  - Skipping final report generation as baseline MAE is not available in V29 flow.")
