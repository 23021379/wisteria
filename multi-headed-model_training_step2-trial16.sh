...
# DEFINITIVE V31.0 ASYMMETRIC ORCHESTRATOR
def train_regional_arbiter_ensemble_v31(df_main_raw, initial_feature_sets, NON_FEATURE_COLS, output_dir):
    """
    [V31.0 ARCHITECTURE]
    ...
    """
    print(f"\n--- V31.0 ASYMMETRIC ARBITER TRAINING INITIATED ---")
    ...
    # Auto-refinement could be added here for the generalist if needed
    ...
    # --- Step 3: Independent Pipeline for the SPECIALIST (with FIREWALL) ---
    ...
    # Create a firewalled initial feature set for the specialist by removing contraband from L0 specialists.
    specialist_initial_feature_sets = {
        head: [f for f in features if not any(c in f for c in CONTRABAND_AVM_FEATURES)]
        for head, features in initial_feature_sets.items()
    }

    specialist_feature_sets = specialist_initial_feature_sets.copy()
    # Auto-refinement could be added here for the specialist if needed, using its firewalled sets

    specialist_all_candidates = [f for f in df_specialist_train.columns if f not in NON_FEATURE_COLS and not any(c in f for c in CONTRABAND_AVM_FEATURES)]
    ...
    # --- Step 4: Arbiter Training (Assembling the SELF-CONTAINED artifacts) ---
    print("\n  - Stage 4: Assembling final artifacts for Arbiter training...")
    ...
    # --- Step 6: Assemble Final Model Artifact ---
    trained_models = {
        'generalist_ensemble': generalist_models, 'specialist_ensemble': specialist_models,
        'arbiter_model': final_arbiter_model, 
        'arbiter_feature_names': X_arbiter.columns.tolist(), # Fulfill the state contract
        'architecture_version': 'V31.0_Asymmetric_Arbiter'
    }
    eval_df = df_main_raw[['property_id', 'most_recent_sale_price']].copy()
    eval_df = df_main_raw[['property_id', 'most_recent_sale_price']].copy()
    eval_df['final_predicted_price'] = np.expm1(final_arbiter_model.predict(X_arbiter))
    
    _generate_and_save_lgbm_diagnostics(final_arbiter_model, X_arbiter, "Arbiter", os.path.join(output_dir, "v31_diagnostics", "arbiter"))
    return trained_models, eval_df

# DEFINITIVE V31.0 PREDICTION ORCHESTRATOR
def predict_on_holdout_v31(df_holdout_raw, trained_models):
    """
    [V31.0 ARCHITECTURE] Predicts using the fully decoupled Asymmetric Arbiter ensemble.
    """
    print("\n--- V31.0: Predicting with Asymmetric Regional Arbiter Ensemble ---")
...
def main():
    ...
    # --- [A.D-31.0] STAGE 6: ASYMMETRIC Data-Driven Feature Selection ---
    ...
    # [A.D-31.0] Define the contraband AVM features the Specialist is forbidden from seeing.
    CONTRABAND_AVM_FEATURES = [
        'num__homipi_estimated_price', # Covers all variants
        'num__mouseprice_estimated_value',
        'num__bricksandlogic_estimated_price',
        'num__INT_avg_estimated_price', # This is an interaction of the above
    ...
    # --- Step 6.1: Select features for the GENERALIST (Full Access) ---
    print("\n--- Selecting features for the GENERALIST model (Full Information Access) ---")
    general_candidate_features = [c for c in df_general_subset.columns if c not in NON_FEATURE_COLS]
    generalist_universal_cols = select_features_with_lgbm(
        df_general_subset, df_general_subset['most_recent_sale_price'], 
        general_candidate_features, n_top_features=250, head_name="Generalist Universal Core"
    )
    joblib.dump(generalist_universal_cols, os.path.join(OUTPUT_DIR, "universal_predictors_generalist_v31.joblib"))

    # --- Step 6.2: Select features for the SPECIALIST (Firewalled) ---
    print("\n--- Selecting features for the SPECIALIST model (AVM Firewall Engaged) ---")
    
    # Create the firewalled candidate list by removing contraband features
    specialist_candidate_pool = [c for c in df_specialist_subset.columns if c not in NON_FEATURE_COLS]
    firewalled_candidate_features = [
        c for c in specialist_candidate_pool 
        if not any(contraband in c for contraband in CONTRABAND_AVM_FEATURES)
    ]
    
    print(f"  - Firewall removed {len(specialist_candidate_pool) - len(firewalled_candidate_features)} contraband AVM features from Specialist's view.")
    
    specialist_universal_cols = select_features_with_lgbm(
        df_specialist_subset, df_specialist_subset['most_recent_sale_price'], 
        firewalled_candidate_features, n_top_features=150, head_name="Specialist Universal Core"
    )
    joblib.dump(specialist_universal_cols, os.path.join(OUTPUT_DIR, "universal_predictors_specialist_v31.joblib"))
    
    print("\n--- STAGE 7: A.D-31.0 Asymmetric Arbiter Protocol Training ---")
    
    trained_models, eval_df = train_regional_arbiter_ensemble_v31(
        df_main_raw, feature_sets, NON_FEATURE_COLS, OUTPUT_DIR
    )
    joblib.dump(trained_models, os.path.join(OUTPUT_DIR, "v31_regional_arbiter_models.joblib"))
    ...
    print(f"--- OOF PERFORMANCE (V31.0 ASSEMBLY): MAE £{final_mae_oof:,.2f} ---")
    eval_df.to_csv(os.path.join(OUTPUT_DIR, "v29_oof_predictions.csv"), index=False)
    
    if not df_holdout_raw.empty:
        holdout_results = predict_on_holdout_v31(df_holdout_raw, trained_models)
        ...
        print(f"--- HOLDOUT SET FINAL RESULTS (V31.0): MAE £{final_mae_holdout:,.2f} ---")
        holdout_results.to_csv(os.path.join(OUTPUT_DIR, "v31_holdout_results.csv"), index=False)
        ...
        print("  - Skipping final report generation as baseline MAE is not available in V31 flow.")

        # --- STAGE 9: V24.0 Intelligence Suite & Dormant Head Manifest ---
        if os.environ.get("WISTERIA_RUN_MODE") == "INTELLIGENCE":
            print("\n--- STAGE 9: Generating V31.0 Intelligence Suite ---")
            ...
            if arbiter_model:
                ...
                report_path = os.path.join(OUTPUT_DIR, "v31_arbiter_feature_importance.csv")
                importance_df.to_csv(report_path, index=False)
                print(f"  - Saved V31 Arbiter feature importance report to {report_path}")
                print("\n--- V31 Arbiter Feature Importances ---")
