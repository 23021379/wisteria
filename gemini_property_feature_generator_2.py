...
# Assume master data is stored here. Path is relative to the main output directory.
MASTER_CONTEXT_DATA_PATH = os.path.join(os.path.dirname(__file__), '..', '[REDACTED_BY_SCRIPT]')
_master_context_df = None

def _load_master_context_data():
    """[REDACTED_BY_SCRIPT]"""
    global _master_context_df
    if _master_context_df is None:
        try:
            # We use 'property_address' as the index for efficient lookups.
            _master_context_df = pd.read_csv(MASTER_CONTEXT_DATA_PATH).set_index('property_address')
            logging.info(f"[REDACTED_BY_SCRIPT]")
        except FileNotFoundError:
            logging.error(f"[REDACTED_BY_SCRIPT]")
            # Create an empty DataFrame to prevent repeated load attempts
            _master_context_df = pd.DataFrame()
        except Exception as e:
            logging.error(f"[REDACTED_BY_SCRIPT]")
            _master_context_df = pd.DataFrame()
    return _master_context_df

# This list is the ONLY source of truth for allowed data, per Directive A-07.17.
AUTHORIZED_FIELDS_SPEC = [
    # == property_vitals ==
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'property_type', 'dtype': str, 'default': "N/A", 'remap': {1: 'Flat', 2: 'House', 0: 'Other/Unknown'}},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'property_sub_type', 'dtype': str, 'default': "N/A", 'remap': {4: 'Detached', 3: 'Semi-Detached', 2: 'Terraced', 1: 'Other'}},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'bedrooms', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'floor_area_sqm', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'construction_year', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'tenure', 'dtype': str, 'default': "N/A", 'remap': {2: 'Freehold', 1: 'Leasehold/Other'}},

    # == local_environment_and_risk ==
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'safety_index_100', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'family_index_100', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': int, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': int, 'default': None},
    ...
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'average_resident_age', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': float, 'default': None},

    # == amenities_and_transport ==
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': str, 'default': "N/A", 'remap': {1: 'Outstanding', 2: 'Good', 3: '[REDACTED_BY_SCRIPT]', 4: 'Inadequate'}},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': '[REDACTED_BY_SCRIPT]', 'dtype': str, 'default': "N/A", 'remap': {1: 'Outstanding', 2: 'Good', 3: '[REDACTED_BY_SCRIPT]', 4: 'Inadequate'}},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'rail_station_dist_km', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'bus_stop_dist_km', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'gp_surgery_dist_km', 'dtype': float, 'default': None},
    {'source': '[REDACTED_BY_SCRIPT]', 'target': 'hospital_dist_km', 'dtype': float, 'default': None},
]

def _format_deprivation(rank):
    """[REDACTED_BY_SCRIPT]"""
    return f"[REDACTED_BY_SCRIPT]" if rank is not None else "N/A"

def generate_property_context_block(property_address: str, year: int) -> str:
    """
    Generates a structured JSON string containing non-monetary context for a property.
    This function strictly adheres to Architectural Directive A-07.17, preventing data leakage.
    """
    df = _load_master_context_data()
    prop_series = None
    if not df.empty and property_address in df.index:
        prop_series = df.loc[property_address]

    # Stage 1: Process the spec to build an intermediate context dictionary
    processed_context = {}
    for spec in AUTHORIZED_FIELDS_SPEC:
        source_col, target_key = spec['source'], spec['target']
        value = default = spec['default']
        if prop_series is not None and source_col in prop_series and pd.notna(prop_series[source_col]):
            try:
                raw_val = prop_series[source_col]
                if 'remap' in spec:
                    value = spec['remap'].get(int(raw_val), default)
                else:
                    value = spec['dtype'](raw_val)
            except (ValueError, TypeError):
                value = default
        processed_context[target_key] = value

    # Stage 2: Assemble the final, structured block
    vitals = {
        "type": processed_context.get('property_type'),
        "sub_type": processed_context.get('property_sub_type'),
        "tenure": processed_context.get('tenure'),
        "bedrooms": processed_context.get('bedrooms') if processed_context.get('bedrooms') is not None else "N/A",
        "floor_area_sqm": processed_context.get('floor_area_sqm') if processed_context.get('floor_area_sqm') is not None else "N/A",
        "construction_year": processed_context.get('construction_year') if processed_context.get('construction_year') is not None else "N/A",
        "epc_rating_current": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
        "epc_rating_potential": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
    }
    environment = {
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
        "safety_index_100": processed_context.get('safety_index_100') if processed_context.get('safety_index_100') is not None else "N/A",
        "family_index_100": processed_context.get('family_index_100') if processed_context.get('family_index_100') is not None else "N/A",
        "deprivation_crime": _format_deprivation(processed_context.get('[REDACTED_BY_SCRIPT]')),
        "[REDACTED_BY_SCRIPT]": _format_deprivation(processed_context.get('[REDACTED_BY_SCRIPT]_decile')),
        "deprivation_environment": _format_deprivation(processed_context.get('[REDACTED_BY_SCRIPT]')),
        "deprivation_health": _format_deprivation(processed_context.get('[REDACTED_BY_SCRIPT]')),
    }
    demographics = {
        "average_resident_age": processed_context.get('average_resident_age') if processed_context.get('average_resident_age') is not None else "N/A",
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
    }
    amenities = {
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]'),
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]') if processed_context.get('[REDACTED_BY_SCRIPT]') is not None else "N/A",
        "[REDACTED_BY_SCRIPT]": processed_context.get('[REDACTED_BY_SCRIPT]'),
        "rail_station_dist_km": processed_context.get('rail_station_dist_km') if processed_context.get('rail_station_dist_km') is not None else "N/A",
        "bus_stop_dist_km": processed_context.get('bus_stop_dist_km') if processed_context.get('bus_stop_dist_km') is not None else "N/A",
        "gp_surgery_dist_km": processed_context.get('gp_surgery_dist_km') if processed_context.get('gp_surgery_dist_km') is not None else "N/A",
        "hospital_dist_km": processed_context.get('hospital_dist_km') if processed_context.get('hospital_dist_km') is not None else "N/A",
    }

    final_block = {
        "property_vitals": vitals,
        "[REDACTED_BY_SCRIPT]": environment,
        "[REDACTED_BY_SCRIPT]": demographics,
        "amenities_and_transport": amenities
    }

    return json.dumps(final_block, indent=2)
